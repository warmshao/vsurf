...
const agentDirEnvName = "VSURF_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "VSURF_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific VSURF_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force VSURF_OFFLINE=1 / VSURF_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.VSURF_OFFLINE = "1";
		env.VSURF_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
...
async function runTuiBenchmarkRun({ runtime, runIndex, measuredIndex, options, profileDir }) {
	...
	const tempRoot = options.isolatedAgentDir ? mkdtempSync(join(tmpdir(), "vsurf-startup-benchmark-")) : undefined;
