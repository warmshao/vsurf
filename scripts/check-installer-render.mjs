...
function findPosixShell() {
	...
	const candidates = [
		...
		join(process.env.ProgramW6432 ?? "C:\\Program Files", "Git", "bin", "sh.exe"),
		"D:\\softwares\\Git\\bin\\sh.exe",
	].filter(Boolean);
	for (const candidate of candidates) {
		if (existsSync(candidate)) return candidate;
	}
	const where = spawnSync("where", ["sh"], { encoding: "utf-8" });
	const fromPath = where.status === 0 ? where.stdout.trim().split(/\r?\n/)[0] : "";
	return fromPath || "sh";
}
...
vsurf_read_terminal_size() {
	vsurf_screen_cols="$vsurf_test_cols"
	vsurf_screen_rows="$vsurf_test_rows"
}
...
print_render_meta() {
	...
	printf '__META__ %s cols=%s rows=%s layout_show_logo=%s lab_width=%s render_lab_width=%s compact=%s visible=%s content_height=%s\\n' \\
		"$label" "$vsurf_screen_cols" "$vsurf_screen_rows" "$vsurf_screen_layout_show_logo" \\
		"$vsurf_screen_layout_lab_width" "$vsurf_screen_render_lab_width" "$vsurf_screen_compact" "$visible" "$content_height"
}

render_case() {
	vsurf_screen_title="Installing VSurf"
	vsurf_screen_detail="Fetching the verified package."
	vsurf_screen_question=
	vsurf_screen_frame=1
	vsurf_screen_cols="$1"
	vsurf_screen_rows="$2"
...
screen_case() {
	...
	vsurf_screen "Installing VSurf" "Installing VSurf" "Fetching the verified package." ""
...
Finalizing npm install."
	for progress_frame in 1 24 25 48 49 200; do
		vsurf_animation_frame="$progress_frame"
		printf '__PROGRESS__ %s\t%s\t%s\\n' "$progress_frame" "$(vsurf_animation_status "Installing VSurf" "$progress_details" static)" "$(vsurf_animation_detail "$progress_details")"
	done
}
...
const tempDir = mkdtempSync(join(tmpdir(), "vsurf-installer-render-"));
...
function runCase(name, initialCols, initialRows, resizedCols, resizedRows) {
	const result = spawnSync(findPosixShell(), [harnessPath, String(initialCols), String(initialRows), String(resizedCols), String(resizedRows)], {
		detached: true,
		encoding: "utf-8",
	});
	if (result.status !== 0) {
...
function assertInstallerProgress(progress) {
	...
	for (const [index, expectedDetail] of expectedDetails.entries()) {
		...
		check(
			progress[index].status === "Installing VSurf...",
