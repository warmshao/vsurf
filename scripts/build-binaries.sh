#!/usr/bin/env bash
#
# Build vsurf binaries for all platforms locally.
...
#     vsurf-darwin-arm64.tar.gz
#     vsurf-darwin-x64.tar.gz
#     vsurf-linux-x64.tar.gz
#     vsurf-linux-arm64.tar.gz
#     vsurf-windows-x64.zip
...
for platform in "${PLATFORMS[@]}"; do
    ...
    if [[ "$platform" == "windows-x64" ]]; then
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/vsurf.exe
    else
        bun build --compile --external koffi --target=bun-$platform ./dist/bun/cli.js --outfile binaries/$platform/vsurf
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == "windows-x64" ]]; then
        # Windows (zip)
        echo "Creating vsurf-$platform.zip..."
        (cd $platform && zip -r ../vsurf-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating vsurf-$platform.tar.gz..."
        mv $platform vsurf && tar -czf vsurf-$platform.tar.gz vsurf && mv vsurf $platform
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf $platform
    if [[ "$platform" == "windows-x64" ]]; then
        mkdir -p $platform && (cd $platform && unzip -q ../vsurf-$platform.zip)
    else
        tar -xzf vsurf-$platform.tar.gz && mv vsurf $platform
    fi
done
