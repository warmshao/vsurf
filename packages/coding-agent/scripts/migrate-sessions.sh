#!/usr/bin/env bash
#
# Migrate sessions from ~/.vsurf/agent/*.jsonl to proper session directories.
# This fixes sessions created by the bug in v0.30.0 where sessions were
# saved to ~/.vsurf/agent/ instead of ~/.vsurf/agent/sessions/<encoded-cwd>/.
#
...
AGENT_DIR="${VSURF_AGENT_DIR:-$HOME/.vsurf/agent}"
DRY_RUN=false
