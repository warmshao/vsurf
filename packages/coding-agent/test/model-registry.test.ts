...
import type { AnthropicMessagesCompat, Api, Context, Model, OpenAICompletionsCompat } from "vsurf-ai";
import { getApiProvider } from "vsurf-ai";
import { getOAuthProvider, registerOAuthProvider } from "vsurf-ai/oauth";
...
describe("ModelRegistry", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-test-model-registry-${Date.now()}-${Math.random().toString(36).slice(2)}`);
