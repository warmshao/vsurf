...
import { Agent, type AgentMessage, type StreamFn } from "vsurf-agent";
import {
...
describe("AgentSession rlm recursion", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-rlm-recursion-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("retries and releases failed retained child cleanup on the next compaction", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(vsurf) => {
				vsurf.on("session_before_compact", async (event) => ({
					compaction: {
	...
	it("lets a stale kernel depth cap defer to the live host gate", () => {
		const python =
			process.env.VSURF_KERNEL_PYTHON ?? join(homedir(), ".vsurf", "agent", "kernel-venv", "bin", "python");
		const runtime = join(process.cwd(), "..", "..", "vsurf-runtime", "src");
...
describe("AgentSession RLM session dir", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-rlm-dir-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
	});
	...
	it("does not create a /tmp dir or set RLM_SESSION_DIR for a non-persisted session", () => {
		...
		const before = readdirSync(tmpdir()).filter((name) => name.startsWith("vsurf-rlm-"));
		...
		const after = readdirSync(tmpdir()).filter((name) => name.startsWith("vsurf-rlm-"));
	...
	it("exports the configured agentDir to the kernel so skills find auth.json", () => {
		...
		const env = (root as unknown as InspectableRlmDirSession)._rlmKernelEnv();
		expect(env.VSURF_CODING_AGENT_DIR).toBe(agentDir);
	});

	it("omits the agentDir env var when none is configured", () => {
		const root = createSession(SessionManager.inMemory(tempDir));
		const env = (root as unknown as InspectableRlmDirSession)._rlmKernelEnv();
		expect(env.VSURF_CODING_AGENT_DIR).toBeUndefined();
	});
