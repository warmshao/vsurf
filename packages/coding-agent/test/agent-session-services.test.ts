...
import { registerFauxProvider } from "vsurf-ai";
...
describe("createAgentSessionFromServices", () => {
	...
	it("shows the telemetry disclosure independently of the Herdr reporter", async () => {
		vi.stubEnv("VSURF_TELEMETRY", "1");
		const tempDir = join(tmpdir(), `vsurf-session-telemetry-notice-${Date.now()}`);
	...
	it("honors an explicit daemon-carried telemetry opt-out", async () => {
		vi.stubEnv("VSURF_TELEMETRY", "1");
		const tempDir = join(tmpdir(), `vsurf-session-daemon-telemetry-opt-out-${Date.now()}`);
		mkdirSync(tempDir, { recursive: true });
		cleanupPaths.push(tempDir);
		const settingsManager = SettingsManager.inMemory();
		const services = await createAgentSessionServices({
	...
	it("does not install top-level telemetry for a resumed child session", async () => {
		vi.stubEnv("VSURF_TELEMETRY", "1");
		const tempDir = join(tmpdir(), `vsurf-session-child-telemetry-${Date.now()}`);
		mkdirSync(tempDir, { recursive: true });
		cleanupPaths.push(tempDir);
		const services = await createAgentSessionServices({
			cwd: tempDir,
	...
	it("forwards daemon-backed agent message controllers into AgentSession", async () => {
		const tempDir = join(tmpdir(), `vsurf-session-services-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
		cleanupPaths.push(tempDir);
	...
	it("hides daemon-backed orchestration skills unless their host bridges are available", async () => {
		const tempDir = join(tmpdir(), `vsurf-session-skills-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		mkdirSync(tempDir, { recursive: true });
		cleanupPaths.push(tempDir);
