...
import { fauxAssistantMessage, registerFauxProvider } from "vsurf-ai";
...
describe("AgentSessionRuntime characterization", () => {
	...
	async function createRuntimeForTest(
		...
		const tempDir =
			options?.cwd ?? join(tmpdir(), `vsurf-runtime-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		const serviceOptions = {
			...
			resourceLoaderOptions: {
				extensionFactories: [
					(vsurf: ExtensionAPI) => {
						vsurf.registerProvider(faux.getModel().provider, {
	...
	it("uses effective runtime depth for a parented new session from a legacy header", async () => {
		const tempDir = join(tmpdir(), `vsurf-runtime-legacy-new-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it.each([false, true])(
		...
		async (inMemory) => {
			const tempDir = join(
				tmpdir(),
				`vsurf-runtime-legacy-fork-${Date.now()}-${Math.random().toString(36).slice(2)}`,
			);
	...
	it("disposes a runtime only once across repeated teardown calls", async () => {
		...
		const { runtime } = await createRuntimeForTest((vsurf: ExtensionAPI) => {
			vsurf.on("session_shutdown", (event) => {
	...
	it("emits session_before_switch and session_start for new and resume flows", async () => {
		...
		const { runtime } = await createRuntimeForTest((vsurf: ExtensionAPI) => {
			vsurf.on("session_before_switch", (event) => {
				events.push(event);
			});
			vsurf.on("session_shutdown", (event) => {
				events.push(event);
			});
			vsurf.on("session_start", (event) => {
				events.push(event);
			});
		});
	...
	it("honors session_before_switch cancellation for new and resume", async () => {
		...
		events.length = 0;
		const otherDir = join(tmpdir(), `vsurf-runtime-other-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	it("emits session_before_fork and session_start and honors cancellation", async () => {
		...
		const { runtime } = await createRuntimeForTest((vsurf: ExtensionAPI) => {
			vsurf.on("session_before_fork", (event) => {
				events.push(event);
				if (cancelNextFork) {
	...
	it("updates the runtime session cwd on cross-cwd session replacement", async () => {
		const firstDir = join(tmpdir(), `vsurf-runtime-cwd-a-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		const secondDir = join(tmpdir(), `vsurf-runtime-cwd-b-${Date.now()}-${Math.random().toString(36).slice(2)}`);
