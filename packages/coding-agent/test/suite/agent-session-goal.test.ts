...
import type { AgentContext, AgentTool } from "vsurf-agent";
import { Agent } from "vsurf-agent";
import { type AssistantMessage, fauxAssistantMessage, fauxToolCall, type Usage } from "vsurf-ai";
...
describe("AgentSession goals", () => {
	...
	it("checks goal budget before continuation while event processing is delayed", async () => {
		...
		let didBlock = false;
		const extension: ExtensionFactory = (vsurf) => {
			vsurf.on("message_end", async (event) => {
