...
import { VSURF_META_NAMESPACE } from "../../src/modes/acp/acp-meta.js";
...
async function connectAcp(harness: Harness, existing?: InProcessAgentConnection): Promise<AcpFixture> {
	...
	return {
		...
		metaOf: (key: string) =>
			updates.map((u) => u.update?._meta?.[VSURF_META_NAMESPACE]?.[key]).filter((value) => value !== undefined),
	};
}
...
describe("ACP mode preserves vsurf features", () => {
	...
	it("surfaces heartbeat and schedule changes, which are not session events", async () => {
		...
		const flags = () =>
			updates
				.map((u) => u.update?._meta?.[VSURF_META_NAMESPACE]?.heartbeatsChanged)
	...
	it("reports a cwd mismatch in _meta instead of failing or silently disagreeing", async () => {
		...
		const cwdMeta = (created._meta?.[VSURF_META_NAMESPACE] as { cwd?: unknown } | undefined)?.cwd;
	...
	it("surfaces a real compaction to the ACP client", async () => {
		const harness = await createHarness({
			...
			extensionFactories: [
				(vsurf: any) => {
					vsurf.on("session_before_compact", async (event: any) => ({
						compaction: {
	...
	it("surfaces a real /refine outcome to the ACP client", async () => {
		...
		// (derived from package vsurfConfig, so VSURF_CODING_AGENT_DIR) rather
		...
		const agentDir = mkdtempSync(join(tmpdir(), "vsurf-acp-refine-"));
	...
	it("reports a failure that a mid-turn transcript rebuild moved below the pre-turn message count", async () => {
		...
		expect(JSON.stringify(failure)).toContain("vsurf turn failed");
	...
	it("advertises vsurf capabilities without polluting the ACP object root", async () => {
		const harness = await createHarness();
		const connection = new InProcessAgentConnection(runtimeHostFor(harness.session));
		const toAgent = new TransformStream<Uint8Array, Uint8Array>();
		const toClient = new TransformStream<Uint8Array, Uint8Array>();
