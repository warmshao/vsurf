...
import type { AgentMessage, AgentTool } from "vsurf-agent";
import { Agent } from "vsurf-agent";
import type { FauxModelDefinition, FauxProviderRegistration, FauxResponseStep, Model } from "vsurf-ai";
...
function createTempDir(): string {
	const tempDir = join(tmpdir(), `vsurf-suite-${Date.now()}-${Math.random().toString(36).slice(2)}`);
