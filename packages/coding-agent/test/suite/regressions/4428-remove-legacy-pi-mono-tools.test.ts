...
describe("regression #4428: remove legacy vsurf-mono built-in tools", () => {
	let tempDir: string;
	let agentDir: string;

	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-remove-legacy-tools-${Date.now()}-${Math.random().toString(36).slice(2)}`);
