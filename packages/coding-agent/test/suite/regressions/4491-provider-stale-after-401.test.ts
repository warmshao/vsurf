...
import type { AgentEvent } from "vsurf-agent";
import { type AssistantMessage, fauxAssistantMessage } from "vsurf-ai";
...
describe("issue #4491 provider stale after repeated 401", () => {
	...
	it("emits stale auth source tokens for daemon clients after bare 401 auth failures", async () => {
		...
		expect(authStaleEvents[0]?.provider).toBe("vsurf-inference");
		...
		expect(harness.authStorage.getAuthStatus("vsurf-inference")).toEqual({
			configured: false,
			source: "stale",
			label: "expired",
		});
