...
import type { AgentTool, ThinkingLevel } from "vsurf-agent";
import { fauxAssistantMessage, fauxToolCall, type Model } from "vsurf-ai";
...
describe("AgentSession model and extension characterization", () => {
	...
	it("setModel saves the model and emits model_select", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(vsurf) => {
					vsurf.on("model_select", async (event) => {
	...
	it("allows model_select handlers to enqueue user messages without waiting on themselves", async () => {
		const harness = await createHarness({
			...
			extensionFactories: [
				(vsurf) => {
					vsurf.on("model_select", () => {
						vsurf.sendUserMessage("from model_select");
					});
				},
			],
		});
	...
	it("allows extension tool_call handlers to block tool execution", async () => {
		...
		const harness = await createHarness({
			...
			extensionFactories: [
				(vsurf) => {
					vsurf.on("tool_call", async () => ({ block: true, reason: "Blocked by test" }));
				},
			],
		});
		harnesses.push(harness);
	...
	it("bindExtensions emits session_start and reload emits session_shutdown then session_start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.on("session_start", async (event) => {
						lifecycleEvents.push(`start:${event.reason}`);
					});
					vsurf.on("session_shutdown", async (event) => {
