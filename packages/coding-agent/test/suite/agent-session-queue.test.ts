...
import { fauxAssistantMessage, fauxToolCall } from "vsurf-ai";
import {
...
describe("AgentSession queue characterization", () => {
	...
	it("cancels a restart checkpoint while tree navigation owns the commit fence", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.on("session_before_tree", async () => {
	...
	it("keeps queued work paused until tree navigation events settle", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.on("session_tree", async () => treeEvent.promise);
				},
			],
		});
		harnesses.push(harness);
	...
	it("invalidates queued prompt preparation on branch navigation", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.on("before_agent_start", async (event) => {
	...
	it.each([
		...
		async ({
			...
			const harness = await createAutoRefineHarness();
			harnesses.push(harness);
			const previousAgentDir = process.env.VSURF_CODING_AGENT_DIR;
			process.env.VSURF_CODING_AGENT_DIR = `${harness.tempDir}/agent`;
			try {
				...
				if (previousAgentDir === undefined) {
					delete process.env.VSURF_CODING_AGENT_DIR;
				} else {
					process.env.VSURF_CODING_AGENT_DIR = previousAgentDir;
				}
			}
		},
	);
	...
	it("rolls back copied local refinement history against the original local harness state", async () => {
		...
		const previousAgentDir = process.env.VSURF_CODING_AGENT_DIR;
		process.env.VSURF_CODING_AGENT_DIR = `${original.tempDir}/agent`;
		try {
			...
			branched.sessionManager.appendCustomEntry("vsurf.refinement", originalRefinement);
	...
	it("rolls back a local refinement in a non-persisted session via the recorded state path", async () => {
		...
		try {
			...
			harness.sessionManager.appendCustomEntry("vsurf.refinement", seeded);
	...
	it("keeps a legacy scope-less rollback in the global store with global scope", async () => {
		...
		try {
			...
			harness.sessionManager.appendCustomEntry("vsurf.refinement", legacyRefinement);
	...
	it("admits extension commands immediately while completion tracks the handler", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.registerCommand("testcmd", { description: "Test", handler: async () => gate });
					vsurf.registerCommand("fail", {
						description: "Fail",
						handler: async () => {
	...
	it("keeps cleared prompts out of the handoff snapshot during the refine wait", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.on("before_agent_start", async () => {
	...
	it("resolves pre-registered queued and direct agent-message delivery waiters once prompts start", async () => {
		...
		const harness = await createHarness({
			extensionFactories: [
				(vsurf) => {
					vsurf.on("turn_start", async () => blocked.promise);
				},
			],
		});
		harnesses.push(harness);
