...
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel, type Model } from "vsurf-ai";
...
function createRuntimeHost(options: { withAuth: boolean; responseDelayMs: number; model?: Model<any> }): {
	...
	const tempDir = join(tmpdir(), `vsurf-rpc-prompt-${Date.now()}-${Math.random().toString(36).slice(2)}`);
