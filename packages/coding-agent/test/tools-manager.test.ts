...
const toolState = vi.hoisted(() => ({
	toolsDir: `/tmp/vsurf-tools-manager-${process.pid}`,
	platform: "linux",
	architecture: "x64",
...
describe("tools manager", () => {
	...
	afterEach(() => {
		...
		if (originalOffline === undefined) delete process.env.VSURF_OFFLINE;
		else process.env.VSURF_OFFLINE = originalOffline;
		rmSync(toolState.toolsDir, { recursive: true, force: true });
	});
	...
	it("formats actionable platform-specific ripgrep warnings", () => {
		...
		expect(mac).toContain("VSurf and subagents remain available");
	});
});
