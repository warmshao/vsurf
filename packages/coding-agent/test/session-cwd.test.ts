...
describe("session cwd handling", () => {
	...
	it("detects missing session cwd from persisted sessions", () => {
		const fallbackCwd = createTempDir("vsurf-session-cwd-fallback");
		const missingCwd = join(fallbackCwd, "does-not-exist");
		const sessionDir = createTempDir("vsurf-session-cwd-session-dir");
	...
	it("reads the header cwd even when the file starts with a blank line", () => {
		...
		const sessionDir = createTempDir("vsurf-session-cwd-blank-line");
		const sessionFile = join(sessionDir, "session.jsonl");
	...
	it("supports overriding the effective cwd when opening a session", () => {
		const fallbackCwd = createTempDir("vsurf-session-cwd-override");
		const missingCwd = join(fallbackCwd, "does-not-exist");
		const sessionDir = createTempDir("vsurf-session-cwd-override-session-dir");
		const sessionFile = join(sessionDir, "session.jsonl");
		cleanupPaths.push(fallbackCwd, sessionDir);
		writeSessionFile(sessionFile, missingCwd);
	...
	it("uses explicit --cwd as the cwd override when opening --resume", async () => {
		const storedCwd = createTempDir("vsurf-session-cwd-stored");
		const explicitCwd = createTempDir("vsurf-session-cwd-explicit");
		const agentDir = createTempDir("vsurf-session-cwd-agent-dir");
		const sessionDir = createTempDir("vsurf-session-cwd-session-dir");
		const sessionFile = join(sessionDir, "session.jsonl");
	...
	it("rejects an unresolved resume selector without converting it to prompt text", async () => {
		const cwd = createTempDir("vsurf-session-cwd-resume-fallback");
		const agentDir = createTempDir("vsurf-session-cwd-resume-fallback-agent-dir");
		const sessionDir = createTempDir("vsurf-session-cwd-resume-fallback-session-dir");
	...
	it("throws a controlled error before runtime creation when the stored cwd is missing", async () => {
		const fallbackCwd = createTempDir("vsurf-session-cwd-runtime");
		const missingCwd = join(fallbackCwd, "does-not-exist");
		const sessionDir = createTempDir("vsurf-session-cwd-runtime-session-dir");
		const sessionFile = join(sessionDir, "session.jsonl");
		cleanupPaths.push(fallbackCwd, sessionDir);
		writeSessionFile(sessionFile, missingCwd);
