...
describe("SettingsManager", () => {
	...
	describe("preserves externally added settings", () => {
		it("should preserve enabledModels when changing thinking level", async () => {
			...
			// Create SettingsManager (simulates vsurf starting up)
	...
	describe("error tracking", () => {
		it("should collect and clear load errors via drainErrors", () => {
			...
			const projectSettingsPath = join(projectDir, ".vsurf", "agent", "settings.json");
		...
		it("should report a new project error when saving after the load error was drained", async () => {
			const settingsPath = join(projectDir, ".vsurf", "agent", "settings.json");
		...
		it("drains only the requested scope", () => {
			...
			writeFileSync(join(projectDir, ".vsurf", "agent", "settings.json"), "{ invalid project json");
			const manager = SettingsManager.create(projectDir, agentDir);
	...
	describe("project settings directory creation", () => {
		it("should not create .vsurf folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .vsurf folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .vsurf folder that beforeEach created
			rmSync(join(projectDir, ".vsurf", "agent"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .vsurf folder should NOT have been created just from reading
			expect(existsSync(join(projectDir, ".vsurf", "agent"))).toBe(false);

			// Settings should still be loaded from global
			expect(manager.getTheme()).toBe("dark");
		});

		it("should create .vsurf folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .vsurf folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .vsurf folder should NOT exist yet
			expect(existsSync(join(projectDir, ".vsurf", "agent"))).toBe(false);
			...
			// Now .vsurf folder should exist
			expect(existsSync(join(projectDir, ".vsurf", "agent"))).toBe(true);

			// And settings file should be created
			expect(existsSync(join(projectDir, ".vsurf", "agent", "settings.json"))).toBe(true);
		});
	});
