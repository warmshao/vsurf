...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.VSURF_OFFLINE;
		delete process.env.VSURF_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.VSURF_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .vsurf skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .vsurf", async () => {
			const extDir = join(tempDir, ".vsurf", "agent", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".vsurf", "agent"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".vsurf", "agent", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".vsurf", "agent", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".vsurf", "agent", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".vsurf", "agent", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".vsurf", "agent", "prompts");
			mkdirSync(promptsDir, { recursive: true });
		...
		it("should resolve directory with package.json vsurf.extensions in extensions setting", async () => {
			// Create a package with vsurf.extensions in package.json
			...
			// Should find the extensions declared in package.json vsurf.extensions
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".vsurf", "agent");
		...
		it("should dedupe user skill entries when ~/.vsurf/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .vsurf auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".vsurf/agent\n");

			const skillDir = join(tempDir, ".vsurf", "agent", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with vsurf manifest", async () => {
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".vsurf", "agent", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .vsurf settings base", () => {
			...
			const rel = relative(join(tempDir, ".vsurf", "agent"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in vsurf manifest", () => {
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
		...
		it("should respect package.json vsurf.extensions manifest in subdirectories", async () => {
	...
	describe("offline mode and network timeouts", () => {
		it("should update project npm packages using @latest when newer version is available", async () => {
			const installedPath = join(tempDir, ".vsurf", "agent", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@latest", "--prefix", join(tempDir, ".vsurf", "agent", "npm")],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".vsurf", "agent", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".vsurf", "agent", "npm", "node_modules", "project-current");
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".vsurf", "agent", "npm", "node_modules", "example");
