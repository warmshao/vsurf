...
describe("fullscreen mode settings", () => {
	...
	beforeEach(() => {
		...
		savedEnv = process.env.VSURF_FULLSCREEN;
		savedTermProgram = process.env.TERM_PROGRAM;
		delete process.env.VSURF_FULLSCREEN;
	...
	afterEach(() => {
		...
		if (savedEnv === undefined) {
			...
			process.env.VSURF_FULLSCREEN = savedEnv;
		}
	...
	it("VSURF_FULLSCREEN env var overrides the setting in both directions", () => {
		const manager = SettingsManager.create(projectDir, agentDir);
		manager.setFullscreen(false);

		process.env.VSURF_FULLSCREEN = "1";
		expect(manager.getFullscreen()).toBe(true);

		manager.setFullscreen(true);
		process.env.VSURF_FULLSCREEN = "0";
