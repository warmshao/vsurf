...
Skill content here.`,
		...
		it("should ignore extra markdown files in auto-discovered skill dirs", async () => {
			const skillDir = join(agentDir, "skills", "vsurf-skills", "browser-tools");
...
Prompt content.`,
		...
		it("should prefer project resources over user on name collisions", async () => {
			...
			const projectPromptsDir = join(cwd, ".vsurf", "agent", "prompts");
			...
			const projectSkillDir = join(cwd, ".vsurf", "agent", "skills", "collision-skill");
...
Project skill`,
			...
			const projectThemePath = join(cwd, ".vsurf", "agent", "themes", "collision.json");
			mkdirSync(join(agentDir, "themes"), { recursive: true });
			mkdirSync(join(cwd, ".vsurf", "agent", "themes"), { recursive: true });
		...
		it("should load symlinked user and project extensions once", async () => {
			...
			writeFileSync(
				...
				`export default function(vsurf) {
	vsurf.registerCommand("shared", {
		...
		handler: async () => {},
			...
			mkdirSync(agentDir, { recursive: true });
			mkdirSync(join(cwd, ".vsurf", "agent"), { recursive: true });
			symlinkSync(sharedExtDir, join(agentDir, "extensions"), "dir");
			symlinkSync(sharedExtDir, join(cwd, ".vsurf", "agent", "extensions"), "dir");
			...
			expect(extensionsResult.extensions[0].path).toBe(join(cwd, ".vsurf", "agent", "extensions", "shared.ts"));
		});

		it("should keep both extensions loaded when command names collide", async () => {
			const userExtDir = join(agentDir, "extensions");
			const projectExtDir = join(cwd, ".vsurf", "agent", "extensions");
	...
	vsurf.registerCommand("project-only", {
	...
	vsurf.registerCommand("user-only", {
...
Content`,
		...
		it("should discover SYSTEM.md from cwd/.vsurf", async () => {
			const piDir = join(cwd, ".vsurf", "agent");
...
Content`,
	...
	describe("bundled skills", () => {
		...
		it("should let a project skill override the bundled websearch skill", async () => {
			const projectSkillDir = join(cwd, ".vsurf", "agent", "skills", "websearch");
			mkdirSync(projectSkillDir, { recursive: true });
			writeFileSync(
