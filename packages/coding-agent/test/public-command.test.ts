...
vi.mock("../src/package-manager-cli.js", () => ({
	...
	isSelfUpdateSource: (source: string) => source === "self" || source === "vsurf" || source === "vsurf",
}));
...
describe("public command routing", () => {
	...
	it("rejects extra attach operands", async () => {
		...
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("vsurf attach <agent>"));
	});
	...
	it("separates VSurf updates from package updates", async () => {
	...
	it("gives legacy update targets explicit migration guidance", async () => {
		for (const target of ["self", "--self", "vsurf"]) {
			await handlePublicCommand(["update", target]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "vsurf update [--force]"'));
	});
	...
	it("directs legacy package-update forms to the package command", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "vsurf package update [source]"'));
	});
	...
	it("rejects self-update aliases on the package update path", async () => {
		for (const source of ["self", "vsurf", "vsurf"]) {
			await handlePublicCommand(["package", "update", source]);
		}

		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "vsurf update"'));
	});
	...
	it("directs package uninstall to package remove", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Use "vsurf package remove"'));
	...
	it("rejects operands for package list", async () => {
		...
		expect(mocks.packageCommands).toEqual([]);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining("vsurf package list"));
	});
	...
	it("rejects the old daemon hierarchy with migration guidance", async () => {
		...
		expect(process.exitCode).toBe(1);
		expect(console.error).toHaveBeenCalledWith(expect.stringContaining('Run "vsurf help"'));
	});

	it("shows migration guidance when help targets removed commands", async () => {
		const cases: Array<[path: string[], hint: string]> = [
			[["daemon"], 'Run "vsurf help"'],
			[["install"], 'Use "vsurf package install"'],
			[["remove"], 'Use "vsurf package remove"'],
			[["uninstall"], 'Use "vsurf package remove"'],
			[["manage"], 'Use "vsurf agents"'],
			[["app", "update"], 'Use "vsurf update"'],
		];
	...
	it("shows command help when options precede the help flag", async () => {
		...
		expect(console.log).toHaveBeenNthCalledWith(1, expect.stringContaining("vsurf list [--all] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(2, expect.stringContaining("vsurf doctor [--fix] [--json]"));
		expect(console.log).toHaveBeenNthCalledWith(3, expect.stringContaining("vsurf package install <source>"));
