...
describe("InteractiveMode submit handling", () => {
	test.each(["normal Enter", "installed custom editor"])("captures exact rich state for %s", async () => {
		...
		// Both vsurf-tui's normal editor and an installed extension editor call their
...
describe("InteractiveMode startup onboarding warnings", () => {
	...
	const liveModel = { id: "gpt-5.5", provider: "vsurf-inference" } as AgentConnectionModel;
	...
	test("keeps real model restore fallback warnings after onboarding", () => {
		...
		expect(
			getModelFallbackWarningAction.call(
				fakeThis,
				"Could not restore model anthropic/claude-old. Using vsurf-inference/openai/gpt-5.5.",
			),
		).toBe("show");
	});
});
...
describe("InteractiveMode startup onboarding", () => {
	...
	test.each([false, true])(
		...
		async (hasExistingStash) => {
			...
			// vsurf-tui clears and emits onChange before invoking onSubmit.
			editorText = "";
	...
	function createOnboardingHarness(shown: boolean, hasAuth = true): OnboardingFake {
		...
		fakeThis.uiServices = {
			modelRegistry: {
				authStorage: AuthStorage.inMemory(),
				refresh: vi.fn(),
				hasConfiguredAuth: vi.fn(() => hasAuth),
	...
	test("shows onboarding when the current model has no configured auth", () => {
	...
	test("skips onboarding when a model with auth is selected", () => {
		const fakeThis = createOnboardingHarness(true);
	...
	test("persists onboarding before opening the configuration flow", async () => {
		let shown = false;
		let flushed = false;
		const fakeThis = createOnboardingHarness(false, false);
	...
	test("opens the Providers tab without forcing login when no models are available", async () => {
		const fakeThis = createOnboardingHarness(false, false);
		fakeThis.connectionState = createConnectionState({ model: undefined });
...
describe("InteractiveMode splash cwd display", () => {
	test("formats home-relative cwd paths", () => {
		...
		expect(formatSplashCwd(path.join(homedir(), "vsurf", "vsurf"))).toBe("~/vsurf/vsurf");
	});

	test("keeps worktree paths as cwd paths instead of repo branch labels", () => {
		expect(formatSplashCwd(path.join(homedir(), "vsurf", "vsurf", ".worktrees", "improve-onboarding"))).toBe(
			"~/vsurf/vsurf/.worktrees/improve-onboarding",
		);
	});
});
...
describe("InteractiveMode.showLoadedResources", () => {
	...
	function createExtensionFixtures(): ExtensionFixture[] {
		return [
			{
				path: "/tmp/project/.vsurf/extensions/answer.ts",
				sourceInfo: createSourceInfo("/tmp/project/.vsurf/extensions/answer.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.vsurf/extensions",
				}),
			},
			{
				path: "/tmp/project/.vsurf/extensions/local-index/index.ts",
				sourceInfo: createSourceInfo("/tmp/project/.vsurf/extensions/local-index/index.ts", {
					source: "local",
					scope: "project",
					origin: "top-level",
					baseDir: "/tmp/project/.vsurf/extensions",
			...
			{
				path: "/tmp/project/.vsurf/npm/node_modules/vsurf-markdown-preview/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.vsurf/npm/node_modules/vsurf-markdown-preview/extensions/index.ts",
					{
						source: "npm:vsurf-markdown-preview",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.vsurf/npm/node_modules/vsurf-markdown-preview",
					},
				),
			},
			{
				path: "/tmp/project/.vsurf/npm/node_modules/@scope/vsurf-scoped/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.vsurf/npm/node_modules/@scope/vsurf-scoped/extensions/index.ts",
					{
						source: "npm:@scope/vsurf-scoped",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.vsurf/npm/node_modules/@scope/vsurf-scoped",
					},
				),
			},
			{
				path: "/tmp/project/.vsurf/git/github.com/HazAT/vsurf-interactive-subagents/extensions/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.vsurf/git/github.com/HazAT/vsurf-interactive-subagents/extensions/index.ts",
					{
						source: "git:github.com/HazAT/vsurf-interactive-subagents",
						scope: "project",
						origin: "package",
						baseDir: "/tmp/project/.vsurf/git/github.com/HazAT/vsurf-interactive-subagents",
					},
				),
			},
			{
				path: "/tmp/project/.vsurf/git/github.com/HazAT/vsurf-interactive-subagents/extensions/subagents/index.ts",
				sourceInfo: createSourceInfo(
					"/tmp/project/.vsurf/git/github.com/HazAT/vsurf-interactive-subagents/extensions/subagents/index.ts",
					{
						source: "git:github.com/HazAT/vsurf-interactive-subagents",
						scope: "project",
						origin: "package",
...
"[Extensions]
  project
    /tmp/project/.vsurf/extensions/answer.ts
    /tmp/project/.vsurf/extensions/local-index
    git:github.com/HazAT/vsurf-interactive-subagents
      extensions
      extensions/subagents
    npm:@scope/vsurf-scoped
      extensions
    npm:vsurf-markdown-preview
      extensions
  user
  ...
  path
	...
	test("shows context paths relative to cwd while preserving full external paths", () => {
		const home = homedir();
		const cwd = path.join(home, "Development", "vsurf-mono");
		const fakeThis = createShowLoadedResourcesThis({
			quietStartup: false,
			cwd,
			contextFiles: [
				{ path: path.join(home, ".vsurf", "agent", "AGENTS.md") },
		...
		expect(output).toContain("~/.vsurf/agent/AGENTS.md, AGENTS.md");
	...
	test("shows full context paths when expanded", () => {
		...
		const output = renderAll(fakeThis.chatContainer).replace(/\\/g, "/");
		expect(output).toContain("[Context]");
		expect(output).toContain("~/.vsurf/agent/AGENTS.md");
		expect(output).toContain("~/Development/vsurf-mono/AGENTS.md");
		expect(output).not.toContain("~/.vsurf/agent/AGENTS.md, AGENTS.md");
	});
