...
describe("kernel bootstrap", () => {
	beforeEach(async () => {
		...
		tempDir = mkdtempSync(join(tmpdir(), "vsurf-kernel-bootstrap-"));
		process.env.HOME = tempDir;
		process.env.PATH = originalEnv.PATH ?? "";
		delete process.env.VSURF_KERNEL_PYTHON;
		delete process.env.VSURF_KERNEL_VENV;
	...
	it("returns the configured kernel venv directory", () => {
		...
		process.env.VSURF_KERNEL_VENV = venv;

		expect(getKernelVenvDir()).toBe(venv);
	});

	it("bootstraps a missing venv with uv, ipykernel, vsurf-runtime, and default extra packages", async () => {
		...
		expect(log).toContain("vsurf-runtime");
	...
	it("installs sibling Python skill dependencies when package and directory names differ", async () => {
		...
		writeFileSync(
			...
			`[project]
name = "vsurf-skill-attach-image"
version = "0.1.0"
`,
		);
		const dependentSkill = createPythonSkillWithDependency("orchestration-heartbeat", "vsurf-skill-attach-image");
		process.env.VSURF_KERNEL_VENV = venv;
...
`,
	...
	it("uses VSURF_KERNEL_PYTHON as an override contract", async () => {
		...
		process.env.VSURF_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).resolves.toBe(overridePython);
	});

	it("allows VSURF_KERNEL_PYTHON missing Python skill imports", async () => {
		const overridePython = join(tempDir, "override-python");
		const pythonSkill = createPythonSkill();
		writeFakePython(overridePython, ["ipykernel", "rlm", ...DEFAULT_RLM_EXTRA_IMPORT_NAMES]);
		process.env.VSURF_KERNEL_PYTHON = overridePython;
	...
	it("rejects VSURF_KERNEL_PYTHON missing default extra packages", async () => {
		const overridePython = join(tempDir, "override-python");
	...
	it("rejects VSURF_KERNEL_PYTHON with a stale rlm runtime", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, ["ipykernel"]);
		process.env.VSURF_KERNEL_PYTHON = overridePython;

		await expect(ensureKernelPython()).rejects.toThrow(/current vsurf-runtime with callable rlm\.run/);
	});

	it("rejects VSURF_KERNEL_PYTHON with a legacy harness API", async () => {
		const overridePython = join(tempDir, "override-python");
		writeExecutable(
			overridePython,
			[
	...
	it("fails an invalid VSURF_KERNEL_PYTHON without bootstrapping", async () => {
		const overridePython = join(tempDir, "override-python");
		writeFakePython(overridePython, []);
		process.env.VSURF_KERNEL_PYTHON = overridePython;
