...
describe("ACP mode over a real IPython kernel", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-acp-kernel-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
`);
		...
		expect(refined[0]?._meta).toMatchObject({
			[VSURF_META_NAMESPACE]: { refinement: { status: "complete" } },
		});
	});
...
`);
	...
	it("sends an agent-to-agent message from the kernel and surfaces it over ACP", {
		...
		).not.toBe("_VsurfUnavailableSkill");
...
`);
		...
		expect(updates[0]?._meta).toMatchObject({
			[VSURF_META_NAMESPACE]: { agentMessage: { toolCallId: "cell-msg", deliveryStatus: "queued" } },
		});
	});
});
