...
import { type Component, Container, visibleWidth } from "vsurf-tui";
...
describe("feature hint deck", () => {
	...
	it("covers VSurf workflows with capability-focused copy", () => {
		const deck = new FeatureHintDeck(() => 0);
		...
		expect(textById.get("subagents")).toBe("VSurf can delegate tasks to subagents and run them in parallel.");
