...
import { registerOAuthProvider } from "vsurf-ai/oauth";
...
describe("AuthStorage", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-test-auth-storage-${Date.now()}-${Math.random().toString(36).slice(2)}`);
	...
	describe("API key resolution", () => {
		...
		test("ambient environment credentials count as available auth", async () => {
			...
			process.env.AWS_PROFILE = "vsurf-test-profile";
		...
		test("prime inference falls back to Prime CLI config when enabled", async () => {
			const primeConfigPath = join(tempDir, "vsurf-config.json");
			...
			await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBe("vsurf-cli-key");
			expect(authStorage.hasAuth("vsurf-inference")).toBe(true);
			expect(authStorage.getAuthStatus("vsurf-inference")).toEqual({
				configured: false,
				source: "vsurf_cli",
				label: "Prime CLI",
			});
		...
		test("prime cli config changes are picked up without reload", async () => {
			...
			await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBe("vsurf-cli-key");
			writeFileSync(primeConfigPath, JSON.stringify({ api_key: "changed-vsurf-key" }));
			await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBe("changed-vsurf-key");
		});
		...
		test("prime inference marks current Prime CLI auth stale", async () => {
			...
			expect(authStorage.markAuthStale("vsurf-inference")).toBe(true);

			expect(authStorage.hasAuth("vsurf-inference")).toBe(false);
			await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBeUndefined();
			expect(authStorage.getAuthStatus("vsurf-inference")).toEqual({
				configured: false,
				source: "stale",
				label: "expired",
		...
		test("setPrimeInferenceApiKey clears stale Prime CLI auth marker", async () => {
			...
			authStorage.setPrimeInferenceApiKey("new-vsurf-key");

			expect(authStorage.hasAuth("vsurf-inference")).toBe(true);
			await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBe("new-vsurf-key");
			expect(authStorage.getAuthStatus("vsurf-inference")).toEqual({
				configured: false,
				source: "vsurf_cli",
				label: "Prime CLI",
		...
		test("prime inference uses environment auth over Prime CLI and stored auth", async () => {
			...
			const originalPrimeTeamId = process.env.VSURF_TEAM_ID;
			process.env.VSURF_API_KEY = "env-vsurf-key";
			delete process.env.VSURF_TEAM_ID;
			try {
				const primeConfigPath = join(tempDir, "vsurf-config.json");
				writeFileSync(
					primeConfigPath,
					JSON.stringify({ api_key: "vsurf-cli-key", team_id: "cli-team", team_name: "CLI Research" }),
				);
				writeAuthJson({
					"vsurf-inference": {
						type: "api_key",
				...
				await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBe("env-vsurf-key");
				expect(authStorage.getAuthStatus("vsurf-inference")).toEqual({
					configured: false,
					source: "environment",
					label: "VSURF_API_KEY",
				});
				expect(authStorage.getProviderHeaders("vsurf-inference")).toBeUndefined();
		...
		test("prime inference provider headers use selected Prime CLI team", () => {
			...
			expect(authStorage.getProviderHeaders("vsurf-inference")).toEqual({ "X-Prime-Team-ID": "cli-team" });
		...
		test("prime inference legacy personal selection suppresses Prime CLI team with Prime CLI key", () => {
			const primeConfigPath = join(tempDir, "vsurf-config.json");
			writeFileSync(primeConfigPath, JSON.stringify({ api_key: "vsurf-cli-key", team_id: "cli-team" }));
			writeAuthJson({
				"vsurf-inference": {
					type: "api_key",
					key: "agent-key",
		...
		test("prime inference environment team overrides legacy personal selection", () => {
			...
			try {
				...
				expect(authStorage.getProviderHeaders("vsurf-inference")).toEqual({ "X-Prime-Team-ID": "env-team" });
				expect(authStorage.getPrimeInferenceTeamSelection()).toBeUndefined();
			} finally {
				if (originalPrimeTeamId === undefined) {
					delete process.env.VSURF_TEAM_ID;
		...
		test("prime inference provider header changes are picked up without reload", () => {
			const primeConfigPath = join(tempDir, "vsurf-config.json");
			writeFileSync(primeConfigPath, JSON.stringify({ api_key: "vsurf-cli-key", team_id: "team-1" }));
			writeAuthJson({});
			...
			expect(authStorage.getProviderHeaders("vsurf-inference")).toEqual({ "X-Prime-Team-ID": "team-1" });
			writeFileSync(primeConfigPath, JSON.stringify({ api_key: "vsurf-cli-key", team_id: "team-2" }));
			expect(authStorage.getProviderHeaders("vsurf-inference")).toEqual({ "X-Prime-Team-ID": "team-2" });
		});
		...
		test("setPrimeInferenceApiKey creates Prime CLI config", async () => {
			...
			expect(config.api_key).toBe("new-vsurf-key");
			expect(statSync(primeConfigPath).mode & 0o777).toBe(0o600);
			expect(authStorage.has("vsurf-inference")).toBe(false);
			await expect(authStorage.getApiKey("vsurf-inference")).resolves.toBe("new-vsurf-key");
		});
		...
		test("setPrimeInferenceApiKey preserves Prime CLI team selection for the same key", () => {
			...
			authStorage.setPrimeInferenceApiKey("vsurf-cli-key");
		...
		test("setPrimeInferenceApiKey removes legacy VSurf credential after Prime CLI save", () => {
			const primeConfigPath = join(tempDir, "vsurf-config.json");
			writeAuthJson({
				"vsurf-inference": {
					type: "api_key",
			...
			expect(agentAuth["vsurf-inference"]).toBeUndefined();
			expect(authStorage.has("vsurf-inference")).toBe(false);
		});

		test("setPrimeInferenceApiKey throws when Prime CLI config cannot be written", () => {
			const primeConfigPath = join(tempDir, "vsurf-config-dir");
			...
			expect(() => authStorage.setPrimeInferenceApiKey("new-vsurf-key")).toThrow();
		...
		test("setPrimeInferenceApiKey preserves team selection when Prime CLI config is disabled", () => {
			...
			authStorage = AuthStorage.create(authJsonPath, { useVsurfCliConfig: false });

			authStorage.setPrimeInferenceApiKey("new-vsurf-key");

			expect(authStorage.get("vsurf-inference")).toEqual({
				type: "api_key",
				key: "new-vsurf-key",
				primeTeam: { teamId: "team-1", name: "Research" },
			});
		...
		test("logout clears Prime CLI credentials when enabled", async () => {
			...
			authStorage.logout("vsurf-inference");
