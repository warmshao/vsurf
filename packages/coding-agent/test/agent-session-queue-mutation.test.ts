...
import { type AssistantMessage, type AssistantMessageEvent, EventStream, getModel, type ImageContent } from "vsurf-ai";
...
describe("AgentSession queue mutation", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-queue-mutation-test-${Date.now()}-${Math.random()}`);
