...
describe("attach-image skill over the kernel host bridge", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-attach-image-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
display({"application/vnd.vsurf.attachment+json": {"mime_type": "image/png", "data": "A" * 10_000_001}}, raw=True)
print("done")
`);
