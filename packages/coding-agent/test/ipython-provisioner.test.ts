...
import { cleanupSessionResources } from "vsurf-ai";
...
const savedForkFlag = process.env.VSURF_KERNEL_FORKSERVER;
beforeAll(() => {
	process.env.VSURF_KERNEL_FORKSERVER = "0";
});
afterAll(() => {
	if (savedForkFlag === undefined) delete process.env.VSURF_KERNEL_FORKSERVER;
	else process.env.VSURF_KERNEL_FORKSERVER = savedForkFlag;
});
...
describe("IpythonKernelProvisioner", () => {
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "vsurf-provisioner-"));
	});
...
describe("KernelManager session cleanup during startup", () => {
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "vsurf-kernel-cleanup-"));
	});
