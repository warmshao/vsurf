...
describe("IPython RLM bootstrap", () => {
	...
	it("disables colored output for subprocesses launched by the kernel", () => {
		expect(buildRlmBootstrapCode()).toContain('_vsurf_os.environ["NO_COLOR"] = "1"');
	});
	...
	it("guards Python skill imports so a broken skill does not abort bootstrap", () => {
		...
		expect(code).toContain("except Exception as _vsurf_skill_error");
		expect(code).toContain("_VsurfUnavailableSkill");
...
describeIfKernel("IPython RLM bootstrap (real kernel)", () => {
	const dir = mkdtempSync(join(tmpdir(), "vsurf-bootstrap-"));
