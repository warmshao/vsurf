...
describe("AgentSession concurrent prompt guard", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `vsurf-concurrent-test-${Date.now()}`);
	...
	it("should queue extension-origin steering messages while streaming", async () => {
		...
		const extensionsResult = await createTestExtensionsResult([
			(vsurf) => {
				(globalThis as typeof globalThis & { testExtensionApi?: unknown }).testExtensionApi = vsurf;
			},
			(vsurf) => {
		...
		expect(vsurf).toBeDefined();

		vsurf!.sendUserMessage("Steer from extension", { deliverAs: "steer" });
