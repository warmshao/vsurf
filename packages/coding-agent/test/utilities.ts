...
import { Agent } from "vsurf-agent";
import { getModel, type OAuthCredentials, type OAuthProvider } from "vsurf-ai";
...
// ============================================================================
// OAuth API key resolution from ~/.vsurf/agent/auth.json
// ============================================================================

const AUTH_PATH = join(homedir(), ".vsurf", "agent", "auth.json");
...
/**
 * Resolve API key for a provider from ~/.vsurf/agent/auth.json
 *
...
/**
 * Check if a provider has credentials in ~/.vsurf/agent/auth.json
 */
...
/** Path to the real vsurf agent config directory */
export const VSURF_AGENT_DIR = join(homedir(), ".vsurf", "agent");

/**
 * Get an AuthStorage instance backed by ~/.vsurf/agent/auth.json
...
export function createTestSession(options: TestSessionOptions = {}): TestSessionContext {
	const tempDir = join(tmpdir(), `vsurf-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
