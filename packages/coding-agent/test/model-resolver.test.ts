...
describe("resolveModelScopeFromModels", () => {
	test("resolves scope patterns against the supplied model list", () => {
		...
		const result = resolveModelScopeFromModels(
			["vsurf-inference/daemon-only-model:high", "openai/gpt-4o"],
...
describe("default model selection", () => {
	test("openai defaults track current models", () => {
		...
		expect(defaultModelPerProvider["vsurf-inference"]).toBe("z-ai/glm-5.2");
	});
	...
	test("findInitialModel prefers GLM 5.2 when VSurf Inference is configured", async () => {
	...
	test("findInitialModel uses another provider default when VSurf Inference is not configured", async () => {
		const anthropicModel: Model<"anthropic-messages"> = {
			...mockModels[0],
			id: "claude-opus-4-7",
			name: "Claude Opus 4.7",
	...
	test("findInitialModel skips saved defaults without configured auth", async () => {
		...
		const registry = {
			...
			hasConfiguredAuth: (model: Model<"anthropic-messages">) => model.provider === "vsurf-inference",
		...
		expect(result.model?.provider).toBe("vsurf-inference");
	...
	test("findInitialModel rebuilds a saved default missing from the model snapshot when the provider is authed", async () => {
		...
		const result = await findInitialModel({
			scopedModels: [],
			isContinuing: false,
			defaultProvider: "vsurf-inference",
