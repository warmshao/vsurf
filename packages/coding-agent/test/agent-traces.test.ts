...
import { PRIME_INFERENCE_PROVIDER_ID, VSURF_TRACES_PROVIDER_ID } from "../src/core/vsurf-inference-auth.js";
...
describe("agent trace upload", () => {
	...
	beforeEach(() => {
		...
		originalTraceApiKey = process.env.VSURF_TRACES_API_KEY;
		originalPrimeApiKey = process.env.VSURF_API_KEY;
		originalTraceBaseUrl = process.env.VSURF_TRACES_BASE_URL;
		originalPrimeBaseUrl = process.env.PRIME_API_BASE_URL;
		delete process.env.VSURF_TRACES_API_KEY;
	...
	afterEach(() => {
		...
		if (originalTraceApiKey === undefined) {
			...
			process.env.VSURF_TRACES_API_KEY = originalTraceApiKey;
		}
	...
	it("does not upload when trace sharing is disabled", async () => {
		...
		const result = await uploadAgentTraceFile({
			...
			authStorage: AuthStorage.inMemory({
				[VSURF_TRACES_PROVIDER_ID]: { type: "api_key", key: "trace-key" },
			}),
	...
	it("uses the production trace API unless a trace-specific base URL is configured", async () => {
		...
		const configPath = join(tempDir, "vsurf-config.json");
	...
	it("uses VSURF_TRACES_BASE_URL for trace API overrides", async () => {
	...
	it("prefers the vsurf-inference credential over the vsurf-cli config key", async () => {
