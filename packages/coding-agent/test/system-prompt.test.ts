...
describe("buildRlmPrompt", () => {
	test("builds the rlm prompt without recursion", () => {
		const prompt = buildRlmPrompt({
			cwd: "/repo",
			messagesPath: "/repo/.vsurf/sessions/session.jsonl",
...
describe("buildSystemPrompt", () => {
	test("injects compact global harness context and refine guidance by default", () => {
			...
			entries: {
				...
				memory: {
					validation: {
						...
						kind: "memory",
						title: "Validation",
						content: "Run `npm run check` after Vsurf code changes.",
						path: "repo/vsurf",
						reference: {},
						arguments: {},
						metadata: {},
		...
		expect(prompt).toContain("Local continual harness entries belong to this VSurf session");
		...
		expect(prompt).toContain("[global:validation] Validation (repo/vsurf, v2): Run `npm run check`");
	...
	test("uses the model-agnostic rlm harness prompt", () => {
		...
		expect(prompt).toContain("Conversation log: /repo/.vsurf/sessions/session.jsonl");
