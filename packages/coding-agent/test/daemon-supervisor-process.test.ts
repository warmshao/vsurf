...
const PROCESS_STRESS_WORKERS = Number.parseInt(process.env.VSURF_STRESS_WORKERS ?? "10", 10);
...
function tempDir(): string {
	const directory = mkdtempSync(join(tmpdir(), "vsurf-daemon-supervisor-test-"));
...
describe("daemon supervisor resident workers", () => {
	it("lists, creates, and attaches passive children through their owning worker", async () => {
		...
		const socketPath = join(tmpdir(), `vsurf-supervisor-passive-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
...
`,
	...
	it("keeps client-owned workers hidden and removes them without archiving", async () => {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-owned-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		...
		const created = await client.request({
			...
			launchEnv: { VSURF_OWNED_TEST: launchEnvSentinel },
			config: { cwd: projectDir, agentDir, sessionDir, noTools: true, noExtensions: true },
		});
		expect(created.success).toBe(true);
	...
	it("releases an adopted client-owned worker when disposal races supervisor replacement", async () => {
		...
		const agentDir = join(root, "agent");
		const projectDir = join(root, "project");
		const socketPath = join(tmpdir(), `vsurf-supervisor-owned-adopt-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
	...
	it("delivers agent-origin messages between root siblings on separate workers", async () => {
		...
		const socketPath = join(
			tmpdir(),
			`vsurf-supervisor-root-message-${process.pid}-${randomUUID().slice(0, 8)}.sock`,
		);
		mkdirSync(projectDir, { recursive: true });
	...
	it("cancels an archived session heartbeat without spawning a worker", { tags: ["process-stress"] }, async () => {
		...
		const socketPath = join(
			tmpdir(),
			`vsurf-supervisor-archived-cron-${process.pid}-${randomUUID().slice(0, 8)}.sock`,
		);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
	...
	it("cancels an orphan heartbeat instead of recreating a descriptorless active session", {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-orphan-cron-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
	...
	it("restarts an empty supervisor without requiring a resident worker", async () => {
		...
		const socketPath = join(tmpdir(), `vsurf-supervisor-restart-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
	...
	it("survives a worker process spawn error", { tags: ["process-stress"] }, async () => {
		...
		const socketPath = join(tmpdir(), `vsurf-supervisor-spawn-error-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
	...
	it("archives resident roots and cancels their heartbeats on explicit daemon shutdown", async () => {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-shutdown-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
	...
	it("finalizes a timed-out worker stop by force-stopping the process and removing its registration", {
		...
		const socketPath = join(
			tmpdir(),
			`vsurf-supervisor-stop-finalize-${process.pid}-${randomUUID().slice(0, 8)}.sock`,
		);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
	...
	it("resumes a saved session immediately after a worker stop fails and the process dies", {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-resume-heal-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
	...
	it("does not resurrect an intentionally stopped root when the supervisor dies during kill", {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-stop-race-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
	...
	it("hosts and adopts isolated worker processes", async () => {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-smoke-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
	...
	it("hosts resident roots in isolated worker processes without a session cap", {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-many-roots-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
	...
	it("isolates a root, streams a chunked snapshot, and adopts the same worker after restart", async () => {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-supervisor-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
		...
		expect(readFileSync(sessionFile, "utf8")).toContain("vsurf.worker_recovery");
	...
	it("runs a session-artifact cron job while the supervisor is being replaced", {
		...
		const projectDir = join(root, "project");
		const sessionDir = join(agentDir, "sessions");
		const socketPath = join(tmpdir(), `vsurf-worker-cron-${process.pid}-${randomUUID().slice(0, 8)}.sock`);
		mkdirSync(projectDir, { recursive: true });
		const sessionManager = SessionManager.create(projectDir, sessionDir);
