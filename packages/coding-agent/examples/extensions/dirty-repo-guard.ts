...
async function checkDirtyRepo(
	...
	const { stdout, code } = await vsurf.exec("git", ["status", "--porcelain"]);
...
export default function (vsurf: ExtensionAPI) {
	vsurf.on("session_before_switch", async (event, ctx) => {
		...
		return checkDirtyRepo(vsurf, ctx, action);
	});

	vsurf.on("session_before_fork", async (_event, ctx) => {
		return checkDirtyRepo(vsurf, ctx, "fork");
	});
}
