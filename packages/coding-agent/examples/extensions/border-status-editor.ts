import { CustomEditor, type ExtensionAPI, type ExtensionContext, type KeybindingsManager } from "vsurf";
import type { Component, EditorTheme, TUI } from "vsurf-tui";
import { truncateToWidth, visibleWidth } from "vsurf-tui";
...
export default function (vsurf: ExtensionAPI) {
	...
	vsurf.on("session_start", (_event, ctx) => {
		...
		const refreshBranch = async () => {
			const result = await vsurf.exec("git", ["branch", "--show-current"], { cwd: ctx.cwd }).catch(() => undefined);
		...
		class BorderStatusEditor extends CustomEditor {
			...
			render(width: number): string[] {
				...
				const thinking = vsurf.getThinkingLevel();
