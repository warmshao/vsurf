 ...
 * - ~/.vsurf/agent/presets.json (global)
 * - <cwd>/.vsurf/agent/presets.json (project-local)
 *
 ...
 *
 * Usage:
 * - `vsurf --preset plan` - start with plan preset
...
import { DynamicBorder, getAgentDir } from "vsurf";
import type { Api, Model } from "vsurf-ai";
import { Container, Key, type SelectItem, SelectList, Text } from "vsurf-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".vsurf", "agent", "presets.json");
...
export default function presetExtension(vsurf: ExtensionAPI) {
	...
	async function applyPreset(name: string, preset: Preset, ctx: ExtensionContext): Promise<boolean> {
		...
		if (activePresetName === undefined) {
			originalState = {
				model: ctx.model,
				thinkingLevel: vsurf.getThinkingLevel(),
				tools: vsurf.getActiveTools(),
			};
		}
		...
		if (preset.thinkingLevel) {
			vsurf.setThinkingLevel(preset.thinkingLevel);
		}

		// Apply tools if specified
		if (preset.tools && preset.tools.length > 0) {
			const allToolNames = vsurf.getAllTools().map((t) => t.name);
			...
			if (validTools.length > 0) {
				vsurf.setActiveTools(validTools);
			}
		}
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify("No presets defined. Add presets to ~/.vsurf/agent/presets.json", "warning");
			return;
		}
		...
		if (result === "(none)") {
			...
			if (originalState) {
				if (originalState.model) {
					await vsurf.setModel(originalState.model);
				}
				vsurf.setThinkingLevel(originalState.thinkingLevel);
				vsurf.setActiveTools(originalState.tools);
			} else {
				vsurf.setActiveTools(["ipython"]);
			}
	...
	vsurf.registerShortcut(Key.ctrlShift("u"), {
	...
	vsurf.on("session_start", async (_event, ctx) => {
		...
		const presetFlag = vsurf.getFlag("preset");
	...
	vsurf.on("turn_start", async () => {
		if (activePresetName) {
			vsurf.appendEntry("preset-state", { name: activePresetName });
		}
	});
}
