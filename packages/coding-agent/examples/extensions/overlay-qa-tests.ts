 ...
 *
 * Usage: vsurf --extension ./examples/extensions/overlay-qa-tests.ts
 *
 * Commands:
...
import type { ExtensionAPI, ExtensionCommandContext, Theme } from "vsurf";
import type { Component, OverlayAnchor, OverlayHandle, OverlayOptions, TUI } from "vsurf-tui";
import { matchesKey, truncateToWidth, visibleWidth } from "vsurf-tui";

// Global handle for toggle demo (in real code, use a more elegant pattern)
let globalToggleHandle: OverlayHandle | null = null;

export default function (vsurf: ExtensionAPI) {
	// Animation demo - proves overlays can handle real-time updates (like vsurf-doom would need)
	vsurf.registerCommand("overlay-animation", {
	...
	vsurf.registerCommand("overlay-anchors", {
	...
	vsurf.registerCommand("overlay-margins", {
	...
	vsurf.registerCommand("overlay-stack", {
	...
	vsurf.registerCommand("overlay-overflow", {
	...
	vsurf.registerCommand("overlay-edge", {
	...
	vsurf.registerCommand("overlay-percent", {
	...
	// Test maxHeight
	vsurf.registerCommand("overlay-maxheight", {
	...
	vsurf.registerCommand("overlay-sidepanel", {
	...
	vsurf.registerCommand("overlay-toggle", {
	...
	vsurf.registerCommand("overlay-passive", {
	...
	vsurf.registerCommand("overlay-focus", {
	...
	vsurf.registerCommand("overlay-streaming", {
	...
	private startProcess(): void {
		...
		this.proc = spawn("bash", [
			...
			for i in $(seq 1 100); do
				...
				DIR="/Users/nicobailon/Documents/development/vsurf-mono/packages/coding-agent/src/modes/interactive"
				...
				if [ $((i % 5)) -eq 0 ]; then
					echo -e "  \\033[32m✓ Successfully processed \${i} files in /Users/nicobailon/Documents/development/vsurf-mono\\033[0m"
				fi
				if [ $((i % 7)) -eq 0 ]; then
...
// Animation demo - proves overlays can handle real-time updates like vsurf-doom
class AnimationDemoComponent extends BaseOverlay {
	...
	render(width: number): string[] {
		...
		lines.push(border("│") + padLine(th.fg("dim", " (vsurf-doom uses same approach)")) + border("│"));
		lines.push(border("│") + padLine(``) + border("│"));
