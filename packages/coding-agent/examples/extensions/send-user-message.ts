 ...
 *
 * Demonstrates vsurf.sendUserMessage() for sending user messages from extensions.
 * Unlike vsurf.sendMessage() which sends custom messages, sendUserMessage() sends
...
export default function (vsurf: ExtensionAPI) {
	...
	vsurf.registerCommand("ask", {
		...
		handler: async (args, ctx) => {
			...
			vsurf.sendUserMessage(args);
		},
	});

	// Command that steers the agent mid-conversation
	vsurf.registerCommand("steer", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				vsurf.sendUserMessage(args, { deliverAs: "steer" });
			}
		},
	});

	// Command that queues a follow-up message
	vsurf.registerCommand("followup", {
		...
		handler: async (args, ctx) => {
			...
			if (ctx.isIdle()) {
				...
				vsurf.sendUserMessage(args, { deliverAs: "followUp" });
