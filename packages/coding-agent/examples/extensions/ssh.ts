 ...
 *
 * Usage:
 *   vsurf -e ./ssh.ts --ssh user@host
 *   vsurf -e ./ssh.ts --ssh user@host:/remote/path
 *
 * Requirements:
...
import { type BashOperations, createBashTool, createEditTool, type EditOperations } from "vsurf";
...
export default function (vsurf: ExtensionAPI) {
	vsurf.registerFlag("ssh", { description: "SSH remote: user@host or user@host:/path", type: "string" });
	...
	vsurf.on("session_start", async (_event, ctx) => {
		...
		const arg = vsurf.getFlag("ssh") as string | undefined;
		if (arg) {
			if (arg.includes(":")) {
