...
export default function (vsurf: ExtensionAPI) {
	...
	vsurf.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "vsurf RPC Demo (new session)" : "vsurf RPC Demo");
	...
	vsurf.on("turn_start", async (_event, ctx) => {
		turnCount++;
		ctx.ui.setStatus("rpc-demo", `Turn ${turnCount} running...`);
	});

	vsurf.on("turn_end", async (_event, ctx) => {
	...
	vsurf.registerCommand("rpc-input", {
	...
	vsurf.registerCommand("rpc-editor", {
	...
	vsurf.registerCommand("rpc-prefill", {
