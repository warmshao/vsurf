...
Example extensions for VSurf.

## Usage

```bash
# From the repository root, load an extension with --extension
./vsurf.sh --extension packages/coding-agent/examples/extensions/permission-gate.ts

# Or copy to extensions directory for auto-discovery
cp permission-gate.ts ~/.vsurf/agent/extensions/
```
...
| `send-user-message.ts` | Demonstrates `vsurf.sendUserMessage()` for sending user messages from extensions |
...
| Extension | Description |
|-----------|-------------|
| `mac-system-theme.ts` | Syncs the VSurf theme with macOS dark/light mode |
...
| `event-bus.ts` | Inter-extension communication via `vsurf.events` |
...
| `custom-provider-gitlab-duo/` | GitLab Duo provider using the inherited `vsurf-ai` streaming API through a proxy |
