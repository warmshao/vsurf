...
import type { ExtensionAPI, SessionBeforeSwitchEvent, SessionMessageEntry } from "vsurf";

export default function (vsurf: ExtensionAPI) {
	vsurf.on("session_before_switch", async (event: SessionBeforeSwitchEvent, ctx) => {
		if (!ctx.hasUI) return;
