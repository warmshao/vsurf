...
import type { ExtensionAPI, ExtensionContext, Theme, ToolExecutionMode } from "vsurf";
import { StringEnum } from "vsurf-ai";
import { type Component, matchesKey, Text, truncateToWidth, visibleWidth } from "vsurf-tui";
...
export default function (vsurf: ExtensionAPI) {
	vsurf.on("session_start", async (_event, ctx) => reconstructState(ctx));
	vsurf.on("session_tree", async (_event, ctx) => reconstructState(ctx));
	...
	// -----------------------------------------------------------------------
	vsurf.registerMessageRenderer(MOVE_MESSAGE_TYPE, (message, { expanded }, theme) => {
	...
	// -----------------------------------------------------------------------
	vsurf.registerMessageRenderer(GAME_OVER_MESSAGE_TYPE, (message, _options, theme) => {
		const details = message.details as BoardDetails | undefined;
...
`;
	...
	// /tic-tac-toe command
	// -----------------------------------------------------------------------
	vsurf.registerCommand("tic-tac-toe", {
		...
		handler: async (_args, ctx) => {
			...
			gameActive = true;
			vsurf.setSessionName("Tic-Tac-Toe");
			...
			await ctx.ui.custom<void>((tui, _theme, _kb, done) => {
				component = new TicTacToeComponent(
					...
					(row, col) => {
						...
						vsurf.appendEntry(SAVE_TYPE, getBoardDetails());
