...
import { type AutocompleteItem, type AutocompleteProvider, type AutocompleteSuggestions, fuzzyFilter } from "vsurf-tui";
...
async function resolveGitHubRepo(vsurf: ExtensionAPI, cwd: string): Promise<RepoResolution> {
	const result = await vsurf.exec("git", ["remote", "-v"], { cwd, timeout: 5_000 });
	if (result.code !== 0) {
...
export default function (vsurf: ExtensionAPI): void {
	vsurf.on("session_start", async (_event, ctx) => {
		const resolvedRepo = await resolveGitHubRepo(vsurf, ctx.cwd);
		if (!resolvedRepo.ok) {
