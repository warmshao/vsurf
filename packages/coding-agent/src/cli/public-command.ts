	...
	switch (command) {
		...
		case "update": {
			...
			if (hasLegacySelfTarget && hasLegacyPackageTarget) {
				return fail(
					"VSurf and package updates are now separate.",
...
function rejectRemovedCommand(args: string[]): PublicCommandResult {
	...
	if (command === "daemon") {
		replacement = 'Run "vsurf help" to see the agent commands.';
	} else if (command === "app" && subcommand === "update") {
		replacement = 'Use "vsurf update".';
	} else if (command === "install") {
		replacement = 'Use "vsurf package install".';
	} else if (command === "remove" || command === "uninstall") {
		replacement = 'Use "vsurf package remove".';
	} else if (command === "manage") {
		replacement = 'Use "vsurf agents".';
	}
...
async function runPackage(args: string[]): Promise<PublicCommandResult> {
	...
	if (subcommand === "update") {
		if (
			...
			return fail('Package updates accept only an optional source. Use "vsurf update --force" to update VSurf.');
		}
		if (rest.length > 1) {
			return fail(`Usage: ${APP_NAME} package update [source]`);
		}
		if (rest[0] && isSelfUpdateSource(rest[0])) {
			return fail('Use "vsurf update" to update VSurf.');
		}
