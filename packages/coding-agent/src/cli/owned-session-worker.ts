...
const OWNED_WORKER_ENV = "VSURF_INTERNAL_OWNED_WORKER";
const OWNED_RECOVERY_DESCRIPTOR_ENV = "VSURF_INTERNAL_OWNED_RECOVERY_DESCRIPTOR";
const OWNED_PROFILE_ENV = "VSURF_INTERNAL_OWNED_PROFILE";
...
function isStartupBenchmark(environment: NodeJS.ProcessEnv): boolean {
	const value = environment.VSURF_STARTUP_BENCHMARK?.toLowerCase();
...
export async function runOwnedSessionWorkerFrontend(
	...
	const recoveryDescriptorPath = join(tmpdir(), `vsurf-owned-${process.pid}-${randomUUID().slice(0, 12)}.json`);
	...
	const anonymousRpcIdPrefix = `vsurf-owned-${randomUUID()}`;
...
export async function maybeRunOwnedSessionWorkerFrontend(
	...
	if (!forceLegacyFrontend && process.env.VSURF_INTERNAL_LEGACY_OWNED_WORKER_FRONTEND !== "1") {
		return false;
	}
