...
export class StaleDaemonError extends Error {
	constructor(
		...
		super(
			`An incompatible VSurf daemon is running.\n\n${daemonIdentity}\n` +
...
async function ensureDaemonRunning(socketPath: string, spawnCwd?: string): Promise<void> {
	...
	// a CLI running inside a daemon worker (e.g. a test spawned by the VSurf
	...
	const throwIfFailed = () => {
		...
		if (childFailure.type === "error") {
			throw new Error(`Failed to spawn VSurf daemon: ${childFailure.error.message}.${logTail}`);
		}
		const signal = childFailure.signal ? `, signal ${childFailure.signal}` : "";
		throw new Error(
			`VSurf daemon exited during startup (code ${childFailure.code ?? "unknown"}${signal}).${logTail}`,
		);
	};
...
export function maybeStartDaemonEarly(args: readonly string[]): void {
	const benchmarkFlag = (process.env.VSURF_STARTUP_BENCHMARK ?? "").toLowerCase();
