...
import { type Api, type ImageContent, type Model, modelsAreEqual } from "vsurf-ai";
import { registerBuiltinMcpOAuthProviders } from "vsurf-ai/mcp";
import { ProcessTerminal, setKeybindings, TUI } from "vsurf-tui";
...
// `vsurf agents` opens the agents view directly.
...
const STARTUP_SESSION_LOSS_COPY: DaemonSessionLossCopy = {
	busyDetail(count) {
		...
		return `A background service from a different VSurf version is running with ${count} busy ${noun}. Stopping it will terminate ${pronoun}.`;
	},
	unlistableDetail:
		"A background service from a different VSurf version is running and its sessions could not be listed. Stopping it may terminate active sessions.",
	question: "Stop it and continue?",
	nonTtyHint: 'Run "vsurf shutdown" to stop it, then retry.',
};
...
async function takeOverStaleDaemonOrExit(socketPath: string): Promise<DaemonReadyResult> {
	...
	if (!(await shutdownDaemonAndWait(socketPath))) {
		console.error(
			chalk.red(`Could not stop the background service on ${socketPath}. Run "vsurf shutdown" and retry.`),
		);
		process.exit(1);
	}
...
export async function main(args: string[], options?: MainOptions) {
	...
	const offlineMode = args.includes("--offline") || isTruthyEnvFlag(process.env.VSURF_OFFLINE);
	if (offlineMode) {
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.VSURF_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: VSURF_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
