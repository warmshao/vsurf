...
import type { ImageContent, TextContent, UserMessage } from "vsurf-ai";
...
export function isSelfUpdateSource(source: string): boolean {
	return source === "self" || source === "vsurf" || source === APP_NAME;
}
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of vsurf executable: ${entrypoint}`);
	}
}
...
const UPDATE_RESTART_CONTINUATION_PROMPT =
	"VSurf restarted after an update. Continue the interrupted task from the saved transcript and restored tool/kernel state. Inspect current state before retrying commands when needed.";

const UPDATE_SESSION_LOSS_COPY: DaemonSessionLossCopy = {
	busyDetail(count) {
		const { noun, pronoun } = pluralizeSessions(count);
		return `VSurf has ${count} busy ${noun}. After the update installs, it will stop ${pronoun}, restart its background service, and resume interrupted work.`;
	},
	unlistableDetail:
		"Running agents could not be listed. After the update installs, VSurf will stop resident agents, restart its background service, and resume interrupted work where possible.",
	question: "Continue?",
			...
			const noticeResponse = await client.request(
				{
					...
					activeSessionId,
					message: {
						customType: "vsurf.update_complete",
						content: `VSurf updated to v${VERSION}. This daemon session was restored after the update.`,
						display: true,
