...
export interface StartupNotices {
	/** Newer VSurf version available, if any. */
	newVersion?: string;
...
export async function checkTmuxKeyboardSetup(): Promise<string | undefined> {
	...
	if (extendedKeysFormat === "xterm") {
		return "tmux extended-keys-format is xterm. Vsurf works best with csi-u. Add `set -g extended-keys-format csi-u` to ~/.tmux.conf and restart tmux.";
	}
