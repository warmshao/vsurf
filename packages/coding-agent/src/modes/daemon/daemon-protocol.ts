...
import type { ImageContent, ServiceTier, TextContent, Transport } from "vsurf-ai";
import type {
...
export const DAEMON_PROTOCOL_NAME = "vsurf.daemon";
 ...
 * that session's vsurf.exec() subprocesses — it does not mutate the daemon's own
...
export function collectDaemonLaunchEnv(source: NodeJS.ProcessEnv = process.env): Record<string, string> {
	...
	for (const [key, value] of Object.entries(source)) {
		if (value !== undefined && !key.startsWith("VSURF_INTERNAL_")) {
			env[key] = value;
		}
	}
	return env;
