...
import { getLogger } from "vsurf-ai";
...
function workerSocketPath(supervisorSocketPath: string, workerId: string): string {
	...
	if (process.platform === "win32") {
		return `\\\\.\\pipe\\vsurf-worker-${key}-${workerId.slice(0, 12)}`;
	}
...
export class DaemonSupervisor {
	...
	async start(): Promise<void> {
		try {
			...
			this.log(`VSurf daemon supervisor ${this.generation} listening on ${this.socketPath}`);
			this.markReady();
		} catch (error) {
