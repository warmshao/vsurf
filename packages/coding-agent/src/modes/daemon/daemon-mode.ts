...
import { type Api, getLogger, type Model } from "vsurf-ai";
...
const UPDATE_RESTART_MARKER =
	"<vsurf_update_interrupted>\n" +
	"VSurf was updated and intentionally interrupted this session. Continue from the saved transcript and restored tool/kernel state. Any running model, tool, bash, or child-agent work may have been partially completed.\n" +
	"</vsurf_update_interrupted>";
...
export class AgentDaemon {
	...
	async start(): Promise<void> {
		...
		this.log(`VSurf daemon listening on ${this.socketPath}`);
	...
	private async createRuntime(
		...
		const createState = async (): Promise<ActiveSessionState> => {
			...
			if (existing) {
				...
				// vsurf.exec disagree with those captures.
				if (command.name) {
					await this.setStateSessionName(existing, command.name);
				}
				this.adoptClientEnv(existing, clientEnv);
