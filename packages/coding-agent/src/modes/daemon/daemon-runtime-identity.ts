...
export const VSURF_BUILD_ID_ENV = "VSURF_BUILD_ID";
export const VSURF_LAUNCHER_PATH_ENV = "VSURF_LAUNCHER_PATH";
...
export function getDaemonRuntimeIdentity(environment: NodeJS.ProcessEnv = process.env): DaemonRuntimeIdentity {
	...
	const launcher = environment[VSURF_LAUNCHER_PATH_ENV];
	return {
		buildId: environment[VSURF_BUILD_ID_ENV] ?? bundledBuildId() ?? `release-${VERSION}`,
