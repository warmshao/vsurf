import type { AssistantMessageEvent } from "vsurf-ai";
import type { AgentConnectionSessionEvent } from "../agent-connection/types.js";
import type { VsurfIpythonMeta, VsurfSessionMeta } from "./acp-meta.js";
import { vsurfMeta } from "./acp-meta.js";

/**
 * Translate vsurf session events into ACP `session/update` payloads.
 *
 * Kept as a pure function so the mapping is testable without a live ACP client
 * or a running agent. Returning an array lets one vsurf event fan out to
...
/** vsurf's model-facing tool is IPython; bash is the secondary escape hatch. */
 ...
 */
function ipythonRichOutput(result: unknown): VsurfIpythonMeta | undefined {
	if (!result || typeof result !== "object") return undefined;
	const details = (result as { details?: unknown }).details;
	if (!details || typeof details !== "object") return undefined;
	const { attachments, diffs } = details as { attachments?: unknown; diffs?: unknown };
	const meta: VsurfIpythonMeta = {};
...
export function acpUpdatesForSessionEvent(
	...
	switch (event.type) {
			...
			return [
				{
					...
					...(rich ? { _meta: vsurfMeta({ ipython: rich }) } : {}),
				},
			];
		}
		...
		// vsurf concepts with no ACP counterpart. They are still part of a
		...
		case "refine_failed":
			return [
				{
					sessionUpdate: "session_info_update",
					_meta: vsurfMeta({ refinement: { status: "failed", error: event.error } }),
				},
			];
...
const BASH_TOOL_CALL_PREFIX = "vsurf-bash";
