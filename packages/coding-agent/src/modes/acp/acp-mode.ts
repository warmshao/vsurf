...
import type { ImageContent } from "vsurf-ai";
...
/**
 * Split ACP prompt blocks into the text and images vsurf accepts.
 *
...
export async function runAcpModeWithConnection(
	...
	const handle = acp
		...
		.onRequest("initialize", async () => ({
			...
			agentInfo: { name: "vsurf", title: "VSurf", version: VERSION },
			// Advertise vsurf extras under a namespaced key: ACP reserves
		...
		.onRequest("session/new", async (ctx: any) => {
			...
			if (session) {
				throw new Error(
					"vsurf ACP mode hosts one session per connection; " + "start another vsurf process for a second session",
				);
			}
			// vsurf's cwd is fixed at startup by the session it was launched
			...
			// Subscribe for the session lifetime, not per prompt turn: vsurf
			...
			const unsubscribe = connection.subscribe((event) => {
				...
				if (event.type === "heartbeats_changed") {
					notify({ sessionUpdate: "session_info_update", _meta: vsurfMeta({ heartbeatsChanged: true }) });
					return;
				}
			...
			return {
				sessionId,
				...(cwdMismatch ? { _meta: vsurfMeta({ cwd: cwdMismatch }) } : {}),
			};
		})
		.onRequest("session/prompt", async (ctx: any) => {
			...
			try {
				...
				if (failure && !abort.signal.aborted) {
					throw new Error(`vsurf turn failed: ${failure}`);
				}
