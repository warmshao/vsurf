...
import { VSURF_LOGO_SMALL } from "../../themes/vsurf-logo.js";
...
export class BrandSplashHeader implements Component {
	...
	constructor(
		...
		this.logoRaw = (options.logo ?? VSURF_LOGO_SMALL).split("\n");
...
export class InteractiveMode {
	...
	async run(): Promise<InteractiveModeRunResult> {
		...
		const newVersionPromise = ownsGlobalStartupNotices ? checkForNewPiVersion(PACKAGE_NAME, this.version) : undefined;
	...
	private async runOnboardingFlow(): Promise<void> {
		...
		// Never force a provider login at startup: open the configuration menu so
		// the user can pick any provider and model. Fresh installs land on the
		// Providers tab to configure an API key first.
		const availableModels = await this.getModelCandidates();
		await this.showConfigurationMenu(availableModels.length > 0 ? "models" : "providers");
	}
	...
	private stashDraftForAgentsView(): void {
		...
		const existing = [this.promptStash, ...(this.promptStashState.queuedStashes ?? [])].filter(
			(stash): stash is PromptStash => stash !== undefined,
		);
		this.promptStash = { ...this.snapshotPromptStash(text), restoreOnOpen: true };
			...
			const selector = new SettingsSelectorComponent(
				{
					...
					currentTheme: this.settingsManager.getTheme() || "vsurf",
