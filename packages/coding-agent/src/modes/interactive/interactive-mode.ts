...
import type { AgentMessage, ThinkingLevel } from "vsurf-agent";
import {
	type Api,
...
import { BUILTIN_MCP_CATALOG } from "vsurf-ai/mcp";
import type {
	AutocompleteItem,
...
import { resolveVsurfTracesBaseUrl } from "../../core/vsurf-inference-auth.js";
import { resolvePrimeInferencePostLoginModelAction } from "../../core/vsurf-inference-model-selection.js";
...
import { PrimeOnboardingSplashComponent } from "./components/vsurf-onboarding-splash.js";
...
export function updateArgsIncludeSelf(args: readonly string[]): boolean {
	...
	return normalized === "self" || normalized === "vsurf" || normalized === APP_NAME.toLowerCase();
}
...
export class InteractiveMode {
	...
	private openExternalEditor(): void {
		...
		const tmpFile = path.join(os.tmpdir(), `vsurf-editor-${Date.now()}.vsurf.md`);
	...
	private async prepareForModelSelectionAfterLogin(authResult: AuthenticationResult): Promise<boolean> {
		...
		if (action.fallbackModel) {
			try {
				...
				this.showError(
					`VSurf Inference login succeeded, but the default model could not be selected: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		} else if (!selectedModel) {
			this.showError("VSurf Inference login succeeded, but the default GLM 5.2 model is unavailable.");
		}
		...
		switch (result.status) {
			...
			case "failed":
				if (result.statusCode === 404) {
					return "Trace upload endpoint was not found. The platform API may not be deployed yet, or VSURF_TRACES_BASE_URL points at the wrong API.";
				}
	...
	private async handleTracesCommand(text: string): Promise<void> {
		...
		if (command === "status") {
			...
			const credential = await getVsurfTraceCredential(this.modelRegistry.authStorage);
			const state = await this.agentConnection.getState();
			const info = [
				...
				`${theme.fg("dim", "Endpoint:")} ${resolveVsurfTracesBaseUrl()}`,
		...
		if (command === "login") {
			await this.createAuthFlows().runVsurfTracesLogin();
			return;
		}
		...
		if (command === "on" || command === "enable") {
			let credential = await getVsurfTraceCredential(this.modelRegistry.authStorage);
			if (!credential) {
				const authResult = await this.createAuthFlows().runVsurfTracesLogin();
				if (authResult.status !== "success") {
					return;
				}
				credential = await getVsurfTraceCredential(this.modelRegistry.authStorage);
			}
			if (!credential) {
