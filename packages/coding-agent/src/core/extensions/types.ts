...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `vsurf` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "vsurf";
	 *
...
export interface ExtensionContext {
	...
	/** Gracefully shutdown vsurf and exit. Available in all contexts. */
	shutdown(): void;
...
export interface BeforeAgentStartEvent {
	...
	systemPrompt: string;
	/** Structured options used to build the system prompt. Extensions can inspect this to understand what Vsurf loaded without re-discovering resources. */
...
/** Configuration for registering a provider via vsurf.registerProvider(). */
...
export type ExtensionFactory = (vsurf: ExtensionAPI) => void | Promise<void>;
...
export interface ExtensionRuntimeState {
	...
	/**
	 * Extra env vars merged over process.env for vsurf.exec() subprocesses (an
...
/**
 * Action implementations for vsurf.* API methods.
