 ...
 * the extension that `herdr integration install pi` writes, so VSurf
...
/**
 * True when Herdr's own file-based Vsurf integration (`herdr integration
 ...
 * paths outside the discovery dirs such as the legacy `~/.vsurf/agent/`) never
...
export function createHerdrAgentStateExtension(getLoadedExtensionPaths: () => string[]): ExtensionFactory {
	return (vsurf: ExtensionAPI) => {
		herdrAgentStateExtensionImpl(vsurf, getLoadedExtensionPaths);
	};
}

/** Built-in reporter with no file-based deferral, for tests and embedders. */
export const herdrAgentStateExtension: ExtensionFactory = (vsurf: ExtensionAPI) => {
	herdrAgentStateExtensionImpl(vsurf, () => []);
};

function herdrAgentStateExtensionImpl(vsurf: ExtensionAPI, getLoadedExtensionPaths: () => string[]): void {
	...
	const unsubscribeBlocked = vsurf.events.on("herdr:blocked", (data: any) => {
		if (!data?.active) {
