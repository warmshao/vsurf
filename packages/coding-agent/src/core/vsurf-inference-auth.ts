...
import type { OAuthAuthInfo } from "vsurf-ai";

export const PRIME_INFERENCE_PROVIDER_ID = "vsurf-inference";
export const PRIME_INFERENCE_PROVIDER_NAME = "VSurf Inference";
export const VSURF_TRACES_PROVIDER_ID = "vsurf-traces";
export const VSURF_TRACES_PROVIDER_NAME = "VSurf Traces";
...
export type PrimeInferenceAuthSource = "vsurf-cli" | "browser";
...
function defaultPrimeCliConfigPath(): string {
	return join(homedir(), ".vsurf", "config.json");
}
...
export function loadPrimeCliConfig(configPath: string = defaultPrimeCliConfigPath()): PrimeCliConfig {
	...
	const teamIdFromEnv = stringEnv("VSURF_TEAM_ID");
...
export function resolveVsurfTracesBaseUrl(baseUrl?: string): string {
	return normalizeBaseUrl(baseUrl ?? stringEnv("VSURF_TRACES_BASE_URL"));
}

function resolveVsurfTracesChallengeConfig(config: PrimeCliConfig): PrimeChallengeConfig {
	return {
		baseUrl: resolveVsurfTracesBaseUrl(),
		frontendUrl: stringEnv("VSURF_TRACES_BASE_URL") ? config.frontendUrl : DEFAULT_PRIME_FRONTEND_URL,
	};
}
...
async function fetchWithTimeout(
	...
	try {
		...
		if (controller.signal.aborted) {
			throw new Error("VSurf Inference request timed out");
		}
		throw error;
	} finally {
		clearTimeout(timeout);
...
export async function checkVsurfTracesAccess(
	apiKey: string,
	baseUrl: string,
	options: {
		fetchFn?: typeof fetch;
...
export async function loginPrimeInference(
	...
	if (config.apiKey) {
		...
		if (access.ok) {
			...
			return { apiKey: config.apiKey, source: "vsurf-cli" };
		}
		callbacks.onProgress?.(
			`Existing Prime CLI key cannot access VSurf Inference (${formatAccessFailure(access)}). Starting browser login...`,
		);
	} else {
	...
	throwIfCancelled(callbacks.signal);
	callbacks.onProgress?.("Checking VSurf Inference access...");
	...
	if (!access.ok) {
		throw new Error(`Prime API key does not have VSurf Inference access (${formatAccessFailure(access)})`);
	}
...
export async function loginVsurfTraces(
	callbacks: PrimeInferenceLoginCallbacks,
	options: PrimeInferenceLoginOptions = {},
): Promise<PrimeInferenceLoginResult> {
	const config = loadPrimeCliConfig(options.configPath);
	const traceConfig = resolveVsurfTracesChallengeConfig(config);
	const fetchFn = options.fetchFn ?? fetch;
	const requestTimeoutMs = options.requestTimeoutMs ?? DEFAULT_REQUEST_TIMEOUT_MS;
	const pollIntervalMs = options.pollIntervalMs ?? DEFAULT_POLL_INTERVAL_MS;
	...
	if (config.apiKey) {
		callbacks.onProgress?.("Checking existing Prime CLI credentials...");
		const access = await checkVsurfTracesAccess(config.apiKey, traceConfig.baseUrl, {
			fetchFn,
			requestTimeoutMs,
			signal: callbacks.signal,
		});
		...
		callbacks.onProgress?.(
			`Existing Prime CLI key cannot upload VSurf traces (${formatAccessFailure(access)}). Starting browser login...`,
		);
	} else {
		callbacks.onProgress?.("No Prime CLI API key found. Starting browser login...");
	}
	...
	throwIfCancelled(callbacks.signal);
	callbacks.onProgress?.("Checking VSurf trace access...");
	const access = await checkVsurfTracesAccess(apiKey, traceConfig.baseUrl, {
		fetchFn,
		requestTimeoutMs,
		signal: callbacks.signal,
	});
	if (!access.ok) {
		throw new Error(`Prime API key does not have VSurf trace access (${formatAccessFailure(access)})`);
	}
