...
import type { ThinkingLevel } from "vsurf-agent";
import { type Api, getLogger, type KnownProvider, type Model, modelsAreEqual } from "vsurf-ai";
import { isValidThinkingLevel } from "../cli/args.js";
import { APP_NAME } from "../config.js";
import { DEFAULT_THINKING_LEVEL } from "./defaults.js";
import type { ModelRegistry } from "./model-registry.js";
import { isPrivatePrimeInferenceModel } from "./vsurf-inference-models.js";
...
export const defaultModelPerProvider: Record<KnownProvider, string> = {
	...
	"vsurf-inference": PRIME_INFERENCE_DEFAULT_MODEL_ID,
...
function findPreferredDefaultModel(availableModels: Model<Api>[]): Model<Api> | undefined {
	const primeInferenceDefault = availableModels.find(
		(model) => model.provider === "vsurf-inference" && model.id === PRIME_INFERENCE_DEFAULT_MODEL_ID,
	);
...
export async function findInitialModel(options: {
	...
	if (defaultProvider && defaultModelId) {
		...
		// build's snapshot (e.g. vsurf-inference catalog churn), so it survives updates.
		const found =
