...
import type { Model } from "vsurf-ai";
import { completeSimple } from "vsurf-ai";
import { getAgentDir } from "../../config.js";
import { serializeConversation } from "../compaction/utils.js";
import { convertToLlm } from "../messages.js";
import type { CustomEntry } from "../session-manager.js";

export const REFINEMENT_CUSTOM_TYPE = "vsurf.refinement";
...
const REFINEMENT_SYSTEM_PROMPT = `You are VSurf's /refine continual harness subsystem.
...
skills, and subagent specs that lets VSurf improve reusable behavior
...
- The default editable continual harness store is local to the current VSurf session. Use it for session-specific progress, active task state, current-run coordination notes, temporary blockers, and project facts that should not affect other sessions.
...
const AUTO_REFINE_REVIEW_SYSTEM_PROMPT = `You are VSurf's automatic /refine review gate.
...
export function formatHarnessStateForPrompt(
	...
	const lines = [
		...
		"",
		"Local continual harness entries belong to this VSurf session. Global continual harness entries persist across VSurf sessions.",
