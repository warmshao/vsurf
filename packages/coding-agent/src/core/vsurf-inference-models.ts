...
export function isPrivatePrimeInferenceModel(model: Pick<Model<string>, "provider" | "id">): boolean {
	return model.provider === "vsurf-inference" && model.id.startsWith("internal/");
}
...
export async function fetchAuthorizedPrivatePrimeInferenceModelIds(
	...
	if (!response.ok) {
		throw new Error(`VSurf Inference model catalog request failed with status ${response.status}`);
	}

	const payload = (await response.json()) as unknown;
	if (!payload || typeof payload !== "object" || !("data" in payload) || !Array.isArray(payload.data)) {
		throw new Error("VSurf Inference model catalog response is invalid");
	}
