...
import type { ServiceTier, Transport } from "vsurf-ai";
...
export interface Settings {
	...
	bundledSkills?: BundledSkillsSettings; // Configure built-in skills shipped with VSurf
	enableBuiltinSkills?: boolean; // default: true - load built-in skills shipped with vsurf
...
export class SettingsManager {
	...
	getClearOnShrink(): boolean {
		...
		return process.env.VSURF_CLEAR_ON_SHRINK === "1";
	}
	...
	getFullscreen(): boolean {
		...
		if (process.env.VSURF_FULLSCREEN !== undefined) {
			return process.env.VSURF_FULLSCREEN === "1";
		}
	...
	getShowHardwareCursor(): boolean {
		return this.settings.showHardwareCursor ?? process.env.VSURF_HARDWARE_CURSOR === "1";
	}
