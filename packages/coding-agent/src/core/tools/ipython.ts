...
import type { AgentTool } from "vsurf-agent";
import type { ImageContent, TextContent } from "vsurf-ai";
...
import asyncio
import os as _vsurf_os

_vsurf_os.environ["NO_COLOR"] = "1"
get_ipython().colors = "nocolor"

try:
    import nest_asyncio as _vsurf_nest_asyncio
    _vsurf_nest_asyncio.apply()
except Exception:
    pass

try:
    import rlm as _vsurf_rlm_module
    rlm = _vsurf_rlm_module.rlm
except Exception as _vsurf_rlm_error:
    _VSURF_RLM_IMPORT_ERROR = str(_vsurf_rlm_error)

    class _VsurfMissingRlm:
        def _raise_missing(self):
            raise RuntimeError(
                "vsurf-runtime is not installed in this IPython kernel. "
                "Remove ~/.vsurf/agent/kernel-venv so vsurf can rebuild it, or set "
                "VSURF_KERNEL_PYTHON to a kernel environment with vsurf-runtime installed. "
                f"Import error: {_VSURF_RLM_IMPORT_ERROR}"
            )
...
import importlib as _vsurf_importlib
import inspect as _vsurf_inspect
import sys as _vsurf_sys
import types as _vsurf_types

class _VsurfCallableSkillModule(_vsurf_types.ModuleType):
    async def __call__(self, *args, **kwargs):
        result = self.run(*args, **kwargs)
        if _vsurf_inspect.isawaitable(result):
            return await result
        return result
...
class _VsurfUnavailableSkill:
    def __init__(self, name, error):
        self.__name__ = name
        self._vsurf_import_error = error
    ...
    async def run(self, *args, **kwargs):
        raise RuntimeError(
            ...
            f"Import error: {self._vsurf_import_error}"
        )
...
def _vsurf_wrap_skill_module(module):
    run = getattr(module, "run", None)
    if not callable(run):
        return module
    if isinstance(module, _VsurfCallableSkillModule):
        return module
    wrapped = _VsurfCallableSkillModule(module.__name__)
    wrapped.__dict__.update(module.__dict__)
    try:
        wrapped.__signature__ = _vsurf_inspect.signature(run)
    except Exception:
        pass
    doc = getattr(run, "__doc__", None)
    if doc:
        wrapped.__doc__ = doc
    _vsurf_sys.modules[module.__name__] = wrapped
    return wrapped

_VSURF_SKILL_IMPORT_ERRORS = {}

for _vsurf_skill_name in ${JSON.stringify(importNames)}:
    try:
        globals()[_vsurf_skill_name] = _vsurf_wrap_skill_module(
            _vsurf_importlib.import_module(_vsurf_skill_name)
        )
    except Exception as _vsurf_skill_error:
        _VSURF_SKILL_IMPORT_ERRORS[_vsurf_skill_name] = str(_vsurf_skill_error)
        globals()[_vsurf_skill_name] = _VsurfUnavailableSkill(
            _vsurf_skill_name,
            str(_vsurf_skill_error),
        )
`.trim();
}
...
export function createIpythonToolDefinition(
	...
	return {
		...
		// The kernel is single-threaded — vsurf must not run two ipython calls in parallel within a batch.
