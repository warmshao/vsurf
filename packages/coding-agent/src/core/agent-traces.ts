...
export type AgentTraceCredentialSource = "environment" | "stored" | "vsurf-inference" | "vsurf-cli";
...
export async function previewAgentTraceFile(options: AgentTracePreviewOptions): Promise<AgentTracePreviewResult> {
	...
	const baseUrl = resolveVsurfTracesBaseUrl(options.baseUrl);
...
export async function getVsurfTraceCredential(
	authStorage: AuthStorage,
	options: { reloadAuth?: boolean; configPath?: string } = {},
): Promise<AgentTraceCredential | undefined> {
	const traceEnvKey = stringEnv("VSURF_TRACES_API_KEY");
	if (traceEnvKey) {
		return { apiKey: traceEnvKey, source: "environment", label: "VSURF_TRACES_API_KEY" };
	}
	...
	const traceKey = await authStorage.getApiKey(VSURF_TRACES_PROVIDER_ID, { includeFallback: false });
	if (traceKey) {
		return { apiKey: traceKey, source: "stored", label: "VSurf Traces credential" };
	}

	const primeEnvKey = stringEnv("VSURF_API_KEY");
	if (primeEnvKey) {
		return { apiKey: primeEnvKey, source: "environment", label: "VSURF_API_KEY" };
	}
	...
	if (primeCredential) {
		...
		if (primeKey) {
			return { apiKey: primeKey, source: "vsurf-inference", label: "VSurf Inference credential" };
		}
	}

	const primeCliKey = loadPrimeCliConfig(options.configPath).apiKey;
	if (primeCliKey) {
		return { apiKey: primeCliKey, source: "vsurf-cli", label: "Prime CLI credential" };
	}
...
async function performAgentTraceUpload(
	...
	const credential = await getVsurfTraceCredential(options.authStorage, {
