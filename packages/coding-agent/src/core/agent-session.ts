...
export class AgentSession {
	 ...
	 * - Handles extension commands (registered via vsurf.registerCommand) immediately, even during streaming
	...
	private async _applyRefine(
		...
		try {
			...
			this.sessionManager.appendCustomEntry("vsurf.refinement", result);
			this._baseSystemPrompt = this._rebuildSystemPrompt(this.getActiveToolNames());
			this.agent.state.systemPrompt = this._baseSystemPrompt;
			try {
	 ...
	 * vsurf.exec() subprocesses. The function is read at exec time, so a host (e.g.
	...
	private _addWebsearchKeyEnv(env: Record<string, string>): void {
		if (this._agentDir) {
			env.VSURF_CODING_AGENT_DIR = this._agentDir;
		}
	...
	private _createEphemeralRlmSessionDir(): string {
		this._rlmSessionDir = mkdtempSync(join(tmpdir(), "vsurf-rlm-"));
		return this._rlmSessionDir;
	}
