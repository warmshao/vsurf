...
function isOfflineModeEnabled(): boolean {
	const value = process.env.VSURF_OFFLINE;
 ...
 *   5  built-in resource shipped with vsurf (source: "builtin")
 */
...
type SkillDiscoveryMode = "vsurf" | "agents";
...
function collectSkillEntries(
	...
	try {
		...
		for (const entry of dirEntries) {
			...
			const relPath = toPosixPath(relative(root, fullPath));
			if (mode === "vsurf" && dir === root && isFile && entry.name.endsWith(".md") && !ig.ignores(relPath)) {
				entries.push(fullPath);
				continue;
			}
...
function readVsurfManifestFile(packageJsonPath: string): VsurfManifest | null {
	try {
		const content = readFileSync(packageJsonPath, "utf-8");
		const pkg = JSON.parse(content) as { vsurf?: VsurfManifest };
		return pkg.vsurf ?? null;
	} catch {
		return null;
	}
}
...
function resolveExtensionEntries(dir: string): string[] | null {
	...
	if (existsSync(packageJsonPath)) {
		const manifest = readVsurfManifestFile(packageJsonPath);
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
	if (resourceType === "skills") {
		return collectSkillEntries(dir, "vsurf");
	}
...
export class DefaultPackageManager implements PackageManager {
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "vsurf-extensions", private: true };
	...
	private getTemporaryDir(prefix: string, suffix?: string): string {
		const hash = createHash("sha256")
			...
			.digest("hex")
			.slice(0, 8);
		return join(tmpdir(), "vsurf-extensions", prefix, hash, suffix ?? "");
	}
	...
	private collectPackageResources(
		...
		const manifest = this.readVsurfManifest(packageRoot);
		if (manifest) {
			for (const resourceType of RESOURCE_TYPES) {
				const entries = manifest[resourceType as keyof VsurfManifest];
	...
	private readVsurfManifest(packageRoot: string): VsurfManifest | null {
	...
	private addAutoDiscoveredResources(
		...
		addResources(
			"skills",
			[
				...collectAutoSkillEntries(projectDirs.skills, "vsurf"),
		...
		addResources(
			"skills",
			[
				...collectAutoSkillEntries(userDirs.skills, "vsurf"),
		...
		if (this.bundledSkillsDir && this.settingsManager.getEnableBuiltinSkills()) {
			...
			const builtinEntries = collectAutoSkillEntries(this.bundledSkillsDir, "vsurf");
