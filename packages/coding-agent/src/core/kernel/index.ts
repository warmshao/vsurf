...
import { registerSessionResourceCleanup } from "vsurf-ai";
...
export const DIFF_DISPLAY_MIME = "application/vnd.vsurf.diff+json";

/** MIME tag the `attach-image` skill emits media payloads under, via `display_data`. */
export const ATTACHMENT_DISPLAY_MIME = "application/vnd.vsurf.attachment+json";

/** MIME tag the `agent-message` skill emits after sending a message. */
export const AGENT_MESSAGE_DISPLAY_MIME = "application/vnd.vsurf.agent-message+json";
...
function makeConnection(): { info: ConnectionInfo; path: string; tempDir: string } {
	...
	const tempDir = mkdtempSync(join(tmpdir(), "vsurf-kernel-"));
...
export class KernelManager {
	...
	constructor(options: KernelManagerOptions) {
		this.options = {
			...
			username: options.username ?? "vsurf",
		};
	}
