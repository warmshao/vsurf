...
const RESULT_MARKER = "__VSURF_KERNEL_STATE__";
...
try:
    _vsurf_snapshot_state()
finally:
    del _vsurf_snapshot_state
`.trim();
}
...
try:
    _vsurf_restore_state()
finally:
    del _vsurf_restore_state
`.trim();
}
...
try:
    _vsurf_list_state_names()
finally:
    del _vsurf_list_state_names
`.trim();
}
