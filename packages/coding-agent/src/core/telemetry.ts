...
import type { AssistantMessage, Usage } from "vsurf-ai";
...
export function isTelemetryEnabled(settingsManager: SettingsManager): boolean {
	if (parseBooleanOverride(process.env.VSURF_OFFLINE) === true) {
		return false;
	}
	if (parseBooleanOverride(process.env.DO_NOT_TRACK) === true) {
		return false;
	}
	const override = parseBooleanOverride(process.env.VSURF_TELEMETRY);
...
export class TelemetryClient implements TelemetrySink {
	...
	constructor(private readonly options: TelemetryClientOptions) {
		this.endpoint = options.endpoint ?? process.env.VSURF_TELEMETRY_ENDPOINT ?? DEFAULT_TELEMETRY_ENDPOINT;
