> VSurf can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to VSurf's capabilities. Use it to embed VSurf in other applications, build custom interfaces, or integrate with automated workflows.
...
```typescript
import { createAgentSession } from "vsurf";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `vsurf.sendMessage()`.
...
The `Agent` class (from `vsurf-agent`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.vsurf/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.vsurf/agent/extensions/`)
- Project skills:
  - `.vsurf/agent/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.vsurf/agent/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.vsurf/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "vsurf";
...
// Default: uses ~/.vsurf/agent/auth.json and ~/.vsurf/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "vsurf";
...
- If you omit `tools`, VSurf automatically creates them with the correct `cwd`
...
import { createAgentSession, defineTool } from "vsurf";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `vsurf.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `vsurf.registerTool()`.

> See [examples/sdk/05-tools.ts](../examples/sdk/05-tools.ts)

### Extensions

Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.vsurf/agent/extensions/`, `.vsurf/agent/extensions/`, and `settings.json` extension sources.
...
**Event Bus:** Extensions can communicate via `vsurf.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "vsurf";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "vsurf";
...
1. Global: `~/.vsurf/agent/settings.json`
2. Project: `<cwd>/.vsurf/agent/settings.json`
