...
Connect external services (Linear, Notion, …) to VSurf over the
[Model Context Protocol](https://modelcontextprotocol.io).

Consistent with VSurf's single-tool design, MCP integrations are **not**
...
Credentials are stored once in `~/.vsurf/agent/auth.json` under `mcp:<name>`.
...
Add it under `mcpServers` in `~/.vsurf/agent/settings.json` (or project
...
`~/.vsurf/agent/skills/acme/`) with the standard Python-skill layout:
...
`pyproject.toml` (depends on `mcp`, `httpx`, and `vsurf-runtime`):

```toml
[project]
name = "vsurf-skill-acme"
version = "0.1.0"
requires-python = ">=3.10"
dependencies = ["mcp", "httpx", "vsurf-runtime"]
...
- **Custom kernel + name collisions.** The kernel import name is the `server`
  value. On a custom `VSURF_KERNEL_PYTHON` that already has an unrelated PyPI
