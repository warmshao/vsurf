...
VSurf saves conversations as sessions so you can continue work, branch from earlier turns, and revisit previous paths.

## Session Storage

Sessions auto-save to `~/.vsurf/agent/sessions/`. Each session is a JSONL file with a tree structure.

```bash
vsurf --continue          # Continue the most recent session
vsurf --resume [path|id]  # Browse past sessions or resume one directly
vsurf --no-session        # Ephemeral mode; do not save
vsurf --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `vsurf --resume` opens the same picker at startup, and `vsurf --resume <path|id>` resumes a specific session.

An invalid ID exits with the closest unambiguous session ID when one is available. To open the picker and send an initial prompt after selecting a session, separate the prompt with `--`: `vsurf --resume -- "continue this work"`.
...
When available, VSurf uses the `trash` CLI for deletion instead of permanently removing files.
...
Named sessions are easier to find in `/resume` and `vsurf --resume`.
...
When `/tree` switches away from one branch to another, VSurf can summarize the abandoned branch and attach that summary at the new position. This preserves important context from the path you left without replaying the whole branch.
