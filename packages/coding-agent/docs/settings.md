...
VSurf uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.vsurf/agent/settings.json` | Global (all projects) |
| `.vsurf/agent/settings.json` | Project (current directory) |
...
VSurf sends pseudonymous, aggregate usage and performance events to Prime Intellect. These events include version and operating-system category, onboarding outcome and duration, execution mode (`interactive`, `print`, `json`, `rpc`, or `acp`), run outcomes, TTFT and latency, prompt and turn counts, token usage, tool success counts, retries, and compactions.

VSurf does not send prompts, responses, thinking, tool arguments or results, command text, filenames, paths, repository information, environment variables, credentials, raw error messages, hostnames, usernames, emails, or hardware identifiers. A random installation ID is stored as `telemetry.json` in the configured agent directory (normally `~/.vsurf/agent/`).
...
```bash
VSURF_TELEMETRY=0 vsurf
DO_NOT_TRACK=1 vsurf
vsurf --offline
```

`VSURF_TELEMETRY_ENDPOINT` overrides the ingestion endpoint for development and self-hosted deployments.
...
`idleEvictionMinutes` is a global daemon policy and is read only from `~/.vsurf/agent/settings.json`. Set it to a positive number to configure the idle threshold.
...
```json
{ "sessionDir": ".vsurf/agent/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `VSURF_SESSION_DIR`, the legacy `VSURF_CODING_AGENT_SESSION_DIR`, then `sessionDir` in `settings.json`.
...
Paths in `~/.vsurf/agent/settings.json` resolve relative to `~/.vsurf/agent`. Paths in `.vsurf/agent/settings.json` resolve relative to `.vsurf/agent`. Absolute paths and `~` are supported.
...
| `enableBuiltinSkills` | boolean | `true` | Load built-in skills shipped with vsurf |
...
```json
{
  "packages": ["vsurf-skills", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["vsurf-skills"]
}
```

## Project Overrides

Project settings (`.vsurf/agent/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.vsurf/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .vsurf/agent/settings.json (project)
{
