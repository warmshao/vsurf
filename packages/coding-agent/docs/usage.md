...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want VSurf to receive the shortcut.

Configure delivery in [Settings](settings.md) with `steeringMode` and `followUpMode`.

## Sessions

Sessions are saved automatically as flat JSONL files under `~/.vsurf/agent/sessions/`. Each session header records its working directory, which the session picker uses for project-scoped views.

```bash
vsurf -c                  # Continue most recent session
vsurf -r [path|id]        # Browse sessions or resume one directly
vsurf --no-session        # Ephemeral mode; do not save
vsurf --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `vsurf agents`, `vsurf list`, or `vsurf attach <agent>` to find and reattach to running work. `vsurf stop <agent>` stops one root agent, while `vsurf shutdown` stops all workers and the local supervisor.
...
- `.vsurf/agent/SYSTEM.md` for a project
- `~/.vsurf/agent/SYSTEM.md` globally
...
vsurf attach <agent>
vsurf stop <agent>
vsurf rename <agent> <name>
vsurf send <agent> <message>
vsurf schedule <list|add|cancel>
vsurf status
vsurf doctor [--fix]
vsurf shutdown [--force]
...
See [VSurf Packages](packages.md) for package sources and security notes.
...
Use `vsurf session export <file> [output]` to export a session to HTML.
...
```bash
vsurf --no-extensions -e ./my-extension.ts
```
...
|----------|-------------|
| `VSURF_CODING_AGENT_DIR` | Override config directory; default is `~/.vsurf/agent` |
...
| `VSURF_KERNEL_PYTHON` | Use an existing Python environment with `ipykernel` instead of bootstrapping `~/.vsurf/agent/kernel-venv` |
| `VISUAL`, `EDITOR` | External editor for Ctrl+G |

The remaining `VSURF_*` variables are compatibility names still read by the current runtime. They do not change the application name, command, or default `~/.vsurf/agent` configuration path.

## Design Principles

VSurf keeps the model-facing tool surface small while making the IPython runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and VSurf packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

VSurf preserves MIT attribution to vsurf-mono for its upstream lineage, but upstream Vsurf product claims and limitations do not describe the current VSurf architecture.
