> VSurf can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that VSurf loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

VSurf implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent IPython kernel.
...
- [Creating Skills with VSurf](#creating-skills-with-vsurf)
...
- Packages: `skills/` directories or `vsurf.skills` entries in `package.json`
- Settings: `skills` array with files or directories
- CLI: `--skill <path>` (repeatable, additive even with `--no-skills`)
- Built-in: `skills/` shipped with the vsurf package (lowest precedence)

Discovery rules:
- In `~/.vsurf/agent/skills/` and `.vsurf/agent/skills/`, direct root `.md` files are discovered as individual skills
...
VSurf ships with built-in skills that load by default:

- `vsurf-intellect` - Prime Intellect products and workflows via the prime CLI: verifiers environments and the Environments Hub, evaluations (local and hosted), Hosted Training and vsurf-rl, sandboxes, tunnels, VSurf Inference, GPU compute, and storage. Reference docs for each area load on demand from the skill's `references/` directory.
...
```bash
export VSURF_WEBSEARCH_TIMEOUT=45
export VSURF_WEBSEARCH_NUM_RESULTS=5
```
...
```json
{
  "skills": ["-vsurf-intellect/SKILL.md"]
}
```
...
For project-level Claude Code skills, add to `.vsurf/agent/settings.json`:
...
1. At startup, VSurf scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, VSurf exposes `web_search` in IPython. If the module defines `run()`, the module is wrapped as an async callable:

```python
await web_search("vsurf skills")
await web_search.run("vsurf skills")
help(web_search)
```

Python skills are installed editable into the kernel venv during kernel setup. By default this is `~/.vsurf/agent/kernel-venv`; set `VSURF_KERNEL_VENV` to override it. If `pyproject.toml` changes, VSurf rebuilds the kernel venv so dependency changes are picked up.

If you set `VSURF_KERNEL_PYTHON`, VSurf does not install packages into that environment. The Python must already have `ipykernel`, `vsurf-runtime`, and the default runtime packages installed. Missing Python skill imports are disabled with a warning and calling the skill raises a `RuntimeError`.
...
```python
await web_search("vsurf")
!web_search "vsurf" --limit 3
```

## Creating Skills with VSurf

VSurf ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:

```text
Create a project Python-backed skill named release-audit in
.vsurf/agent/skills/release-audit. It should expose
...
1. **Scope:** use `.vsurf/agent/skills/<name>/` for a project skill committed with the repository, or `~/.vsurf/agent/skills/<name>/` for a personal skill.
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh VSurf session after adding a Python-backed skill so kernel setup can install and import the package.
...
VSurf validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
...
- [Vsurf Skills](https://github.com/badlogic/vsurf-skills) - Web search, browser automation, Google APIs, transcription
