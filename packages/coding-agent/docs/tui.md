> VSurf can create TUI components. Ask it to build one for your use case.
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "vsurf-tui";
...
```typescript
import { Container, type Focusable, Input } from "vsurf-tui";
...
**In custom tools** via `vsurf.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = vsurf.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `vsurf-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "vsurf-tui";
```
...
VSurf shows compact image metadata and does not emit terminal graphics for images.
...
```typescript
import { matchesKey, Key } from "vsurf-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "vsurf-tui";
...
```typescript
import { getMarkdownTheme } from "vsurf";
import { Markdown } from "vsurf-tui";
...
Set `VSURF_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
VSURF_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `vsurf-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "vsurf";
import { DynamicBorder } from "vsurf";
import { Container, type SelectItem, SelectList, Text } from "vsurf-tui";
...
```typescript
import { BorderedLoader } from "vsurf";

vsurf.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `vsurf-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "vsurf";
import { Container, type SettingItem, SettingsList, Text } from "vsurf-tui";

vsurf.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while VSurf is streaming a response.
...
// Restore VSurf's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "vsurf-tui";
