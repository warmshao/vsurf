...
Extensions can register custom model providers via `vsurf.registerProvider()`. This enables:
...
export default function (vsurf: ExtensionAPI) {
  ...
  vsurf.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. VSurf waits for the factory before startup continues, so the provider is available during interactive startup and to `vsurf model list`.
...
Use `vsurf.unregisterProvider(name)` to remove a provider that was previously registered via `vsurf.registerProvider(name, ...)`:
...
// Later, remove it
vsurf.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map VSurf levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
vsurf.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "vsurf-ai";
...
Credentials are persisted in `~/.vsurf/agent/auth.json`:
...
interface ProviderModelConfig {
  ...
  /** Maps VSurf thinking levels to provider/model-specific values; null marks a level unsupported. */
