> VSurf can create themes. Ask it to build one for your setup.
...
VSurf loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.vsurf/agent/themes/*.json`
- Project: `.vsurf/agent/themes/*.json`
- Packages: `themes/` directories or `vsurf.themes` entries in `package.json`
...
On first run, VSurf detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.vsurf/agent/themes
vim ~/.vsurf/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/warmshao/vsurf/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, VSurf reloads it automatically for immediate visual feedback.
...
VSurf uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, VSurf falls back to the nearest approximation.
