...
```
~/.vsurf/agent/sessions/<session-id>.jsonl
```
...
Sessions can be removed by deleting their `.jsonl` files under `~/.vsurf/agent/sessions/`.

VSurf also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, VSurf uses the `trash` CLI to avoid permanent deletion.
...
For TypeScript definitions in your project, inspect `node_modules/vsurf/dist/` and `node_modules/vsurf-ai/dist/`.
...
### Base Message Types (from VSurf AI)
...
### Extended Message Types (from VSurf Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by VSurf (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name` command or `vsurf.setSessionName()` in extensions.
