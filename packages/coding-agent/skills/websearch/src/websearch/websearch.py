...
def _agent_dir() -> Path:
    """Resolve the VSurf config dir the same way the runtime does."""
    raw = (
        os.environ.get("VSURF_CODING_AGENT_DIR")
        or os.environ.get("VSURF_CODING_AGENT_DIR")
        or str(Path.home() / ".vsurf" / "agent")
    )
    ...
    *,
    ...
    if not api_key:
        return (
            ...
            "  2. In VSurf, run /login, switch to MCP Connections, choose \"Serper (web search)\", and paste the key.\n"
            "Do not ask the user to set environment variables. Once the key is saved, web search works automatically."
        )

    if timeout is None:
        timeout = _env_int("VSURF_WEBSEARCH_TIMEOUT", 45)
    if num_results is None:
        num_results = _env_int("VSURF_WEBSEARCH_NUM_RESULTS", 5)
