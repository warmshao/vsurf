...
verifiers is Prime Intellect's Python library for LLM environments: packages that expose `load_environment` and bundle datasets, rollout logic, and reward rubrics. Environments power evaluations (local and hosted) and RL training (Hosted Training or self-managed vsurf-rl).

Live docs: `verifiers/overview.md`, `tutorials-environments/getting-started.md`, `hosted-training/getting-started.md`, `vsurf-rl/overview.md` under https://docs.primeintellect.ai/ (append `.md` for raw markdown). Source: https://github.com/PrimeIntellect-ai/verifiers and https://github.com/PrimeIntellect-ai/vsurf-rl
...
- VSurf Inference models include estimated run cost in eval output automatically.
...
- Self-managed path: `prime lab setup --vsurf-rl`, then follow vsurf-rl's own configs and launch commands. Treat vsurf-rl as a power-user path requiring local GPU access; see https://docs.primeintellect.ai/vsurf-rl/overview.md.
