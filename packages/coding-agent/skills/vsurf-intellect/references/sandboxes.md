...
`vsurf_sandboxes` ships `SandboxClient` (sync) and `AsyncSandboxClient` (async) with identical methods:

```python
from vsurf_sandboxes import SandboxClient, CreateSandboxRequest, APIClient
...
```python
from vsurf_tunnel import Tunnel

tunnel = Tunnel(local_port=8000)
await tunnel.start()   # prints the public URL
```

Hosted evaluations can create tunnels too: launch the run with `--allow-tunnel-access` (grants tunnel permission to the temporary `VSURF_API_KEY`), then use the same flow inside the hosted sandbox.
