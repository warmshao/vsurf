...
- Removed the fullscreen image fallback instruction and kept fullscreen overlays from emitting terminal graphics ([#437](https://github.com/warmshao/vsurf/pull/437) by [@snimu](https://github.com/snimu)).
...
- Fixed fullscreen dock selection so prompt text can be copied ([#342](https://github.com/warmshao/vsurf/pull/342)).

## [0.2.8] - 2026-07-09

## [0.2.7] - 2026-07-08

- Fixed full TUI redraws to preserve terminal scrollback on resize and shrink redraws ([#331](https://github.com/warmshao/vsurf/pull/331) by [@sethkarten](https://github.com/sethkarten)).
...
- Added fullscreen viewport rendering with alternate-screen scrolling, follow controls, mouse selection, and clipboard copy hooks ([#316](https://github.com/warmshao/vsurf/pull/316)).
...
- Added `fuzzyFilterScored`, returning scored matches so callers can break ties by recency; `fuzzyFilter` delegates to it unchanged ([#251](https://github.com/warmshao/vsurf/issues/251)).
- Changed the editor to expose `rlmChildLabel` (whitespace-normalized full prompt) and centralized the "still working" pulse glyph in `working-icon.ts` for shared use across panels and lists ([#247](https://github.com/warmshao/vsurf/issues/247)).
...
- Added marquee TUI components and a Prime-branded theme as part of the initial VSurf fork from pi-mono.
...
- Added support for `VSURF_TUI_WRITE_LOG` directory paths, creating a unique log file (`tui-<timestamp>-<pid>.log`) per instance for easier debugging of multiple pi sessions ([#2508](https://github.com/badlogic/pi-mono/pull/2508) by [@mrexodia](https://github.com/mrexodia))
...
- Added `VSURF_DEBUG_REDRAW=1` env var for debugging full redraws (logs triggers to `~/.pi/agent/pi-debug.log`)

### Changed

- Terminal height changes no longer trigger full redraws, reducing flicker on resize
- `clearOnShrink` now defaults to `false` (use `VSURF_CLEAR_ON_SHRINK=1` or `setClearOnShrink(true)` to enable)
...
- Added `VSURF_TUI_WRITE_LOG` environment variable to capture raw ANSI output for debugging
...
- Hardware cursor is now disabled by default for better terminal compatibility. Set `VSURF_HARDWARE_CURSOR=1` to enable (replaces `VSURF_NO_HARDWARE_CURSOR=1` which disabled it).
