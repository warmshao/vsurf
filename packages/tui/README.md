...
```typescript
import { TUI, Text, Editor, ProcessTerminal, matchesKey } from "vsurf-tui";
...
Renders Kitty or iTerm2 terminal graphics when supported. Set `fallbackOnly: true` to show compact image metadata instead; VSurf uses this metadata-only mode.
...
```typescript
import { CombinedAutocompleteProvider } from "vsurf-tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "vsurf-tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "vsurf-tui";
import type { Component } from "vsurf-tui";
...
```bash
VSURF_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
