...
// The full VSurf Inference catalog is registered (minus raw/duplicate variants).
...
function readPrimeCliConfig(): Record<string, unknown> {
	try {
		const parsed = JSON.parse(readFileSync(join(homedir(), ".vsurf", "config.json"), "utf8"));
...
function getPrimeInferenceConfigValue(
	envName: "VSURF_API_KEY" | "VSURF_TEAM_ID",
...
function getExistingPrimeInferenceModels(): Model<"openai-completions">[] {
	const models = EXISTING_MODELS["vsurf-inference"] as unknown as Record<string, Model<"openai-completions">>;
...
async function fetchPrimeInferenceModels(): Promise<Model<"openai-completions">[]> {
	...
	const apiKey = getPrimeInferenceConfigValue("VSURF_API_KEY", primeConfig, ["api_key", "apiKey"]);
	const teamId = getPrimeInferenceConfigValue("VSURF_TEAM_ID", primeConfig, ["team_id", "teamId", "teamID"]);
	let catalog: PrimeInferenceCatalogEntry[] = [];

	try {
		console.log("Fetching models from VSurf Inference API...");
		...
		console.error("Failed to fetch VSurf Inference models:", error);
	}
	...
	try {
		...
		console.error("Failed to fetch OpenRouter catalog for VSurf Inference metadata:", error);
	}
	if (openRouterIndex.size === 0) {
		...
		console.error("OpenRouter catalog unavailable; keeping snapshot VSurf Inference models");
	...
	console.log(`Loaded ${models.length} VSurf Inference models (${catalogModels.length} from the live catalog)`);
	return models;
}
