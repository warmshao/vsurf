...
const primeInferenceApiKey = getEnvApiKey("vsurf-inference");
...
describe("Generate E2E Tests", () => {
	...
	describe.skipIf(!primeInferenceApiKey)("VSurf Inference Provider (openai/gpt-5.5)", () => {
		const llm = getModel("vsurf-inference", "openai/gpt-5.5");
	...
	// =========================================================================
	// OAuth-based providers (credentials from ~/.vsurf/agent/oauth.json)
	...
	describe.skipIf(!hasBedrockCredentials())("Amazon Bedrock Provider (claude-opus-4-6 interleaved thinking)", () => {
		...
		it("should pass requestMetadata to the SDK payload", { retry: 3 }, async () => {
			...
			let capturedPayload: unknown;
			const metadata = { app: "vsurf-test", env: "ci" };
			const response = await complete(
				llmSonnet,
				{
					messages: [
	...
	if (!process.env.VSURF_NO_LOCAL_LLM) {
		try {
