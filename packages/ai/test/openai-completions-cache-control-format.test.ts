...
describe("openai-completions cacheControlFormat", () => {
	...
	it("applies Anthropic-style cache markers for VSurf Inference Anthropic models", async () => {
		const model = getModel("vsurf-inference", "anthropic/claude-fable-5");
		const { params, result } = await runCompletion(model);
		expectAnthropicCacheMarkers(params);
		expect(result.usage.cost.cacheWrite).toBeCloseTo((80 * model.cost.input * 1.25) / 1_000_000);
	});

	it("prices one-hour VSurf Inference cache writes at twice the input rate", async () => {
		const model = getModel("vsurf-inference", "anthropic/claude-fable-5");
	...
	it("does not apply Anthropic-style cache markers to other VSurf Inference models", async () => {
		const model = getModel("vsurf-inference", "openai/gpt-5.6-sol");
		const params = await capturePayload(model);
