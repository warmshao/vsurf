...
const originalPrimeApiKey = process.env.VSURF_API_KEY;

afterEach(() => {
	if (originalPrimeApiKey === undefined) {
		delete process.env.VSURF_API_KEY;
	} else {
		process.env.VSURF_API_KEY = originalPrimeApiKey;
	}
});

describe("VSurf Inference models", () => {
	it("registers the VSurf Inference catalog", () => {
		const modelIds = getModels("vsurf-inference").map((model) => model.id);
	...
	it("marks flagship models as featured so pickers can pin them above the long tail", () => {
		expect(getModel("vsurf-inference", "openai/gpt-5.5").featured).toBe(true);
		expect(getModel("vsurf-inference", "z-ai/glm-5.2").featured).toBe(true);
		expect(getModel("vsurf-inference", "moonshotai/kimi-k3").featured).toBe(true);
		expect(getModel("vsurf-inference", "google/gemini-2.5-pro").featured).toBeUndefined();
		expect(getModel("vsurf-inference", "openai/gpt-4o").featured).toBeUndefined();
	});

	it("uses mandatory provider efforts for Qwen 3.8 Max", () => {
		const model = getModel("vsurf-inference", "qwen/qwen3.8-max");
	...
	it("uses reasoning toggles for models without effort selectors", () => {
		for (const provider of ["vsurf-inference", "openrouter"] as const) {
			const model = getModel(provider, "qwen/qwen3.7-flash");

			expect(model.compat?.supportsReasoningEffort).toBe(false);
			expect(getSupportedThinkingLevels(model)).toEqual(["off", "high"]);
			if (provider === "vsurf-inference") expect(model.compat?.thinkingFormat).toBe("openrouter");
		}
	});

	it("registers Kimi K3 on VSurf Inference and OpenRouter", () => {
		for (const provider of ["vsurf-inference", "openrouter"] as const) {
	...
	it("borrows OpenRouter metadata for non-curated catalog models", () => {
		const gemini = getModel("vsurf-inference", "google/gemini-2.5-pro");
		...
		const nemotronSuper = getModel("vsurf-inference", "nvidia/nemotron-3-super-120b-a12b");
		...
		const maverick = getModel("vsurf-inference", "meta-llama/llama-4-maverick");
	...
	it("registers the default OpenAI-compatible model", () => {
		const model = getModel("vsurf-inference", "openai/gpt-5.5");

		expect(model).toBeDefined();
		expect(model.api).toBe("openai-completions");
		expect(model.provider).toBe("vsurf-inference");
	...
	it("marks known reasoning-capable VSurf Inference model families", () => {
		const opus48 = getModel("vsurf-inference", "anthropic/claude-opus-4.8");
		expect(opus48.reasoning).toBe(true);
		expect(getSupportedThinkingLevels(opus48)).toEqual(["off", "low", "medium", "high", "xhigh", "max"]);

		const sonnet5 = getModel("vsurf-inference", "anthropic/claude-sonnet-5");
		...
		expect(getModel("vsurf-inference", "anthropic/claude-opus-4.7").reasoning).toBe(true);
		const deepseekV4Flash = getModel("vsurf-inference", "deepseek/deepseek-v4-flash");
		expect(deepseekV4Flash.reasoning).toBe(true);
		expect(deepseekV4Flash.compat).toMatchObject({
			requiresReasoningContentOnAssistantMessages: true,
			thinkingFormat: "deepseek",
		});
		const glm51 = getModel("vsurf-inference", "z-ai/glm-5.1");
		expect(glm51.reasoning).toBe(true);
		expect(glm51.compat).toMatchObject({
			supportsReasoningEffort: false,
			thinkingFormat: "zai",
		});
		const glm52 = getModel("vsurf-inference", "z-ai/glm-5.2");
		expect(glm52.reasoning).toBe(true);
		expect(glm52.compat).toMatchObject({
			supportsReasoningEffort: false,
			thinkingFormat: "zai",
		});
		expect(getModel("vsurf-inference", "qwen/qwen3-coder-next").reasoning).toBe(false);
		expect(getModel("vsurf-inference", "x-ai/grok-4.20").reasoning).toBe(true);
		expect(getModel("vsurf-inference", "minimax/minimax-m3").reasoning).toBe(true);
		expect(getModel("vsurf-inference", "moonshotai/kimi-k2.7-code").reasoning).toBe(true);
	});

	it("uses route-specific context windows for VSurf Inference Claude routes", () => {
		const fable = getModel("vsurf-inference", "anthropic/claude-fable-5");
		...
		expect(getModel("vsurf-inference", "anthropic/claude-opus-4.6").contextWindow).toBe(1000000);
		expect(getModel("vsurf-inference", "anthropic/claude-opus-4.7").contextWindow).toBe(1000000);
		expect(getModel("vsurf-inference", "anthropic/claude-opus-4.8").contextWindow).toBe(1000000);
		expect(getModel("vsurf-inference", "anthropic/claude-sonnet-4.6").contextWindow).toBe(1000000);
		expect(getModel("vsurf-inference", "anthropic/claude-sonnet-5").contextWindow).toBe(1000000);
		expect(getModel("vsurf-inference", "anthropic/claude-haiku-4.5").contextWindow).toBe(200000);
		...
		expect(getModel("vsurf-inference", "anthropic/claude-sonnet-4.5").contextWindow).toBe(200000);
	});

	it("resolves VSURF_API_KEY from the environment", () => {
		process.env.VSURF_API_KEY = "test-vsurf-key";

		expect(findEnvKeys("vsurf-inference")).toEqual(["VSURF_API_KEY"]);
		expect(getEnvApiKey("vsurf-inference")).toBe("test-vsurf-key");
	});

	it("requires an explicit VSurf Inference API key", () => {
		delete process.env.VSURF_API_KEY;

		expect(findEnvKeys("vsurf-inference")).toBeUndefined();
		expect(getEnvApiKey("vsurf-inference")).toBeUndefined();
	});
});
