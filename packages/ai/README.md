...
<h1 align="center">VSurf AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the VSurf package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
- **OpenAI**
- **VSurf Inference** (OpenAI-compatible API)
...
```bash
npm install vsurf-ai
```

TypeBox exports are re-exported from `vsurf-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'vsurf-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'vsurf-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'vsurf-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'vsurf-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'vsurf-ai';
...
```typescript
import { getModel, stream } from 'vsurf-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'vsurf-ai';
...
```typescript
import { Model, stream } from 'vsurf-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are VSurf thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'vsurf-ai';
...
```typescript
import { getModel, complete, Context } from 'vsurf-ai';
...
```typescript
import { Context, getModel, complete } from 'vsurf-ai';
...
- OAuth login flows are not supported in browser environments. Use the `vsurf-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'vsurf-ai';
...
```bash
npx vsurf-ai login              # interactive provider selection
npx vsurf-ai login anthropic    # login to specific provider
npx vsurf-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `vsurf-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
```typescript
import { loginGitHubCopilot } from 'vsurf-ai/oauth';
...
```typescript
import { getModel, complete } from 'vsurf-ai';
import { getOAuthApiKey } from 'vsurf-ai/oauth';
...
**VSurf Inference**: Uses the OpenAI-compatible API at `https://api.pinference.ai/api/v1`. Set `VSURF_API_KEY` or pass an API key explicitly.
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `vsurf-ai`
