...
const CALLBACK_HOST = process.env.VSURF_OAUTH_CALLBACK_HOST || "127.0.0.1";
...
const CALLBACK_PORT_BASE = Number(process.env.VSURF_MCP_OAUTH_CALLBACK_PORT || 53700);
...
export function createMcpOAuthProvider(config: McpOAuthConfig): OAuthProviderInterface {
	...
	async function login(callbacks: OAuthLoginCallbacks): Promise<OAuthCredentials> {
		...
		if (!clientId) {
			...
			clientId = await registerClient(meta.registration_endpoint, `VSurf (${label})`);
		}
