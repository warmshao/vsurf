...
// VSURF_TEAM_ID env var, falling back to team_id in ~/.vsurf/config.json.
export function getPrimeTeamId(): string | undefined {
	const fromEnv = process.env.VSURF_TEAM_ID || getProcEnv("VSURF_TEAM_ID");
	if (fromEnv?.trim()) return fromEnv.trim();

	if (!_existsSync || !_readFileSync || !_homedir || !_join) return undefined;
	const configPath = _join(_homedir(), ".vsurf", "config.json");
