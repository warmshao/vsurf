...
- Updated the model catalog snapshot with Claude Opus 5 across supported providers, Gemini 3.5/3.6 Flash models, Kimi K3 variants, and the current 103-model VSurf Inference catalog; removed the discontinued `gpt-5-chat`, `gpt-5-codex`, and `laguna-m.1` routes.
...
- Added Kimi K3 to the VSurf Inference and OpenRouter model catalogs with multimodal input and mandatory max reasoning.
- Fixed prompt caching and retention-aware cache-cost estimates for Anthropic models routed through VSurf Inference ([ENG-4723](https://linear.app/primeintellect/issue/ENG-4723)).

## [0.3.1] - 2026-07-15

- Added provider service-tier forwarding to shared stream options.
- Removed team-gated internal VSurf Inference routes from the public model catalog so clients can discover them from authenticated team catalogs instead.
- Fixed the generated Nemotron 3 Ultra output cap being dropped when OpenRouter omits max completion tokens ([#420](https://github.com/warmshao/vsurf/pull/420)).
...
- Registered the full VSurf Inference catalog (97 models, up from 32) instead of a curated whitelist; context/output limits, vision, and reasoning now come from OpenRouter metadata with a small override table for limits the gateway enforces differently (verified against the live API), and raw/duplicate variants (BF16, HF-cased, `zai-org/`, fine-tune outputs) are skipped.
- Fixed VSurf Inference context windows that disagreed with the live gateway: `anthropic/claude-sonnet-4.5` capped at 200k (route rejects longer prompts), `z-ai/glm-5.2` and `internal/glm-5.2-fast` raised to 1M, `minimax/minimax-m3` corrected to 512k, `nvidia/nemotron-3-*` corrected to their enforced 262k/131k windows.
- Removed `vsurf-intellect/intellect-3`, which no longer serves (404 from the gateway).
- Added an optional `featured` flag to `Model` so pickers can pin flagship models above a provider's long tail; set for 30 VSurf Inference flagships.
...
- Added `internal/glm-5.2-fast` to the VSurf Inference model catalog.
- Added Claude Sonnet 5 to the Anthropic and VSurf Inference model catalogs.

## [0.2.6] - 2026-07-06

## [0.2.5] - 2026-07-06

- Added Claude Fable 5 to the VSurf Inference model catalog and refreshed generated model metadata ([#317](https://github.com/warmshao/vsurf/pull/317)).
- Changed provider stream failures to preserve classified causes, raw stop reasons, and request IDs instead of collapsing them into generic unknown errors ([#313](https://github.com/warmshao/vsurf/pull/313)).

## [0.2.4] - 2026-07-01

- Added Claude Fable 5 support on the Anthropic and Bedrock providers, handling its always-on adaptive thinking by never sending an explicit `thinking: disabled` or sampling params (which Fable rejects with a 400) ([#302](https://github.com/warmshao/vsurf/issues/302)).
- Fixed the `openai-completions` and `openai-responses` providers serializing empty tool results as a literal "(see attached image)" placeholder; the placeholder is now gated on the result actually having images, matching the google-shared and mistral providers ([#290](https://github.com/warmshao/vsurf/issues/290)).

## [0.2.3] - 2026-06-30

- Added a `./mcp` entry point with a built-in MCP server catalog (Linear, Notion) and generic OAuth 2.1 (PKCE + dynamic client registration) providers stored as `mcp:<server>` in `auth.json`; the local OAuth callback server tries a range of ports so a stale or concurrent login can't block sign-in ([#280](https://github.com/warmshao/vsurf/issues/280)).
- Added the `qwen/qwen3-30b-a3b-instruct-2507` model to the VSurf Inference catalog ([#284](https://github.com/warmshao/vsurf/issues/284)).

## [0.2.2] - 2026-06-25

- Added a curated `vision` flag on VSurf Inference model metadata so vision-capable models advertise `["text", "image"]` input, since the VSurf Inference models API reports no modality data ([#261](https://github.com/warmshao/vsurf/issues/261)).
- Changed VSurf Inference model names to drop the redundant ` (VSurf Inference)` suffix at the generator source ([#252](https://github.com/warmshao/vsurf/issues/252)).
- Fixed the declared context window for VSurf Inference Claude Opus 4.6/4.7/4.8 and Sonnet 4.6 to 200k (the route lacks the long-context beta), and added `calculatePromptTokens` so context budgeting counts prompt tokens only ([#246](https://github.com/warmshao/vsurf/issues/246)).
...
- Added the `glm-5.2`, `minimax-m3`, and `kimi-k2.7-code` models from VSurf Inference, and marked the MiniMax `minimax-m*` and Moonshot `kimi*` families as reasoning models in the registry generator.
...
- Added Claude Opus 4.8 to the curated VSurf Inference model list.
...
- Removed the `x-ai/grok-code-fast-1` model from the VSurf Inference catalog.

## [0.0.7] - 2026-06-01

### Fixed

- Fixed OAuth callback pages to use VSurf logo branding on a black background.
...
- Added VSurf Inference as an OpenAI-compatible built-in provider using `VSURF_API_KEY` authentication, curated model generation, and Prime-specific request compatibility.
- Added GLM 5 and GLM 5.1 to the VSurf Inference model catalog with Z.ai-style thinking compatibility.
...
- Lazy-load built-in provider modules and root provider wrappers so importing `@mariozechner/vsurf-ai` no longer eagerly loads provider SDKs, significantly reducing base startup cost without changing dependency installation footprint ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@mariozechner/vsurf-ai/oauth` package exports to point directly at built `dist` files, avoiding broken TypeScript resolution through unpublished wrapper targets ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level package entry. Import OAuth login/refresh functions from `@mariozechner/vsurf-ai/oauth` instead of `@mariozechner/vsurf-ai` ([#1814](https://github.com/badlogic/pi-mono/issues/1814))
...
- Re-exported TypeBox `Type`, `Static`, and `TSchema` from `@mariozechner/vsurf-ai` to match documentation and avoid duplicate TypeBox type identity issues in pnpm setups ([#1338](https://github.com/badlogic/pi-mono/issues/1338))
...
- Added `VSURF_AI_ANTIGRAVITY_VERSION` environment variable to override the Antigravity User-Agent version when Google updates their version requirements ([#1129](https://github.com/badlogic/pi-mono/issues/1129))
...
- Added `VSURF_CACHE_RETENTION` environment variable to control cache TTL for Anthropic (5m vs 1h) and OpenAI (in-memory vs 24h). Set to `long` for extended retention. Only applies to direct API calls (api.anthropic.com, api.openai.com). ([#967](https://github.com/badlogic/pi-mono/issues/967))
...
- Browser compatibility for vsurf-ai: replaced top-level Node.js imports with dynamic imports for browser environments ([#873](https://github.com/badlogic/pi-mono/issues/873))
...
- **Agent API moved**: All agent functionality (`agentLoop`, `agentLoopContinue`, `AgentContext`, `AgentEvent`, `AgentTool`, `AgentToolResult`, etc.) has moved to `@mariozechner/vsurf-agent`. Import from that package instead of `@mariozechner/vsurf-ai`.
